...
import { CHAT_ENDPOINTS } from '../../../../../src/modules/user/social/constants/chat.constants';
...
describe('social chat endpoint paths', () => {
  it('stays relative to the configured /api base URL', () => {
    ...
    expect(CHAT_ENDPOINTS.read(conversationId)).toBe(`/chat/conversations/${conversationId}/read`);
    expect(CHAT_ENDPOINTS.forward(conversationId)).toBe(`/chat/messages/${conversationId}/forward`);
