        ...
        {query.data?.notifications.map((notification) => {
          ...
          return (
            <button key={notification.id} type="button" onClick={() => {
              ...
              <span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-(--text-primary)">{notification.message}</span><span className="mt-1 block text-[11px] text-(--text-secondary)">{formatNotificationDate(notification.createdAt)}</span></span>
