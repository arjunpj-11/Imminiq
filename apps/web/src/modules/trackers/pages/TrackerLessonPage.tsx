...
const DEFAULT_CHAT_GREETING =
  'Hello Scholar! Ask me anything about this lesson.'
...
const formatMathTextToHtml = (value: string) => {
  ...
  html = html.replace(
    /\^\{([^{}]+)\}/g,
    (_match, power: string) => `<sup>${power}</sup>`
  )

  html = html.replace(
    /_\{([^{}]+)\}/g,
    (_match, sub: string) => `<sub>${sub}</sub>`
  )
  ...
  return (
    <div
      ...
      dangerouslySetInnerHTML={{
        __html: formatMathTextToHtml(children),
      }}
    />
  )
}
    ...
    return (
      <div className="flex flex-wrap gap-2">
        ...
        <button
          type="button"
          onClick={() =>
            setMessage('Give me a practical example')
          }
          className="rounded-full border border-[#e0d0c5] px-3 py-1.5 font-['DM_Mono',monospace] text-[9px] font-semibold uppercase tracking-[0.08em] text-[#6b5f58] transition hover:border-[#e8816a] hover:bg-[rgba(184,76,43,0.08)] hover:text-[#b84c2b] dark:border-white/9 dark:text-[#9b9a92] dark:hover:text-[#e8816a]"
        >
          Show examples
...
function ReflectionPracticeCard({
  ...
  const baseQuestions = useMemo(() => {
    const qs = [
      ...
      `Previous-year style question: Why is ${lesson.title} important for ${lesson.lessonType.replace(
        '_',
        ' '
      )} preparation?`,
  ...
  const [verification, setVerification] =
    useState<VerifyLessonAnswerResponse['data'] | null>(null)
...
function CompilerCard({
  ...
  const submitCode = () => {
    submitCodeMutation.mutate(
      ...
      },
      {
        ...
        onError: (error) => {
          setOutput(`> Submit Error: ${error.message}`)
        },
      }
    )
  }
  ...
  const compareOptimized = () => {
    optimizedMutation.mutate(
      ...
      {
        onSuccess: (response) => {
          setOptimizedSolution(response.data)
        },
      }
    )
  }
  ...
  const handleLanguageChange = (
    ...
    setOutput(
      `> Compiler language changed to ${nextLanguage.label}`
    )
      ...
      <div className="space-y-3">
        ...
        {nextLesson ? (
          ...
          <button
            ...
            disabled={completing}
            onClick={onComplete}
            className="w-full rounded-[14px] bg-[#b84c2b] px-4 py-4 text-[13px] font-bold text-[#fdf8f5] shadow-[0_4px_12px_rgba(184,76,43,0.28)] transition hover:-translate-y-px hover:bg-[#963d22] disabled:cursor-wait disabled:opacity-60 dark:bg-[#e8816a] dark:text-[#141412] dark:hover:bg-[#d4705a]"
          >
...
export default function TrackerLessonPage() {
  ...
  const userInitials = getInitials(
    dashboardSummary.user.fullName
  )

  const getReturnStack = () => {
    const state = location.state as LessonLocationState | null

    if (state?.returnToRoadmapStack?.length) {
      return state.returnToRoadmapStack
    }
  ...
  const goToLesson = (id: string) => {
    navigate(`/trackers/${trackerId}/lessons/${id}`, {
      state: {
        returnToRoadmapStack: getReturnStack(),
      },
    })
  }
      ...
      <div className="relative z-1 flex min-h-screen w-full overflow-x-clip">
        ...
        >
          <TopBar
            ...
            userAvatarUrl={
              dashboardSummary.user.avatarUrl || undefined
            }
            userLevel={formatLevelLabel(
              dashboardSummary.user.isPremium
            )}
            isGuest={false}
          />
              ...
              <div className="flex min-w-0 flex-col gap-6">
                <section className="rounded-[20px] border-[1.5px] border-[#e0d0c5] bg-[#fdf8f5] p-6 shadow-[0_2px_16px_rgba(26,23,20,0.06)] dark:border-white/9 dark:bg-[#1e1c19] max-[640px]:p-4.5">
                  <div className="mb-4 font-['DM_Mono',monospace] text-[9px] uppercase tracking-[0.15em] text-[#6b5f58] opacity-60 dark:text-[#9b9a92]">
                    Trackers › {tracker.title} ›{' '}
                    {lessonNode.topicTitle || 'Lesson'}
                  </div>
                ...
                {showCompiler ? (
                  <>
                    ...
                    <CompilerCard
                      ...
                      language={
                        generatedLesson.codeExample.language
                      }
                      fileName={
                        generatedLesson.codeExample.fileName
                      }
                      initialCode={codeForCompiler}
                      practiceTitle={
                        generatedLesson.practiceTask.title
                      }
                      practiceDescription={
                        generatedLesson.practiceTask.description
                      }
