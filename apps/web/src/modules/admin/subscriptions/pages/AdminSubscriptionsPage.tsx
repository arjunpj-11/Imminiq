    ...
    <>
      ...
      <AdminPanel title="Subscription plans">
        <div className="border-b border-[rgba(255,255,255,0.09)] px-6 py-4 text-xs text-[#aaa59d]">
          Prices are stored in paise. Set a usage limit to 0 for unlimited. Free users always receive the latest free plan. Existing paid subscribers keep purchased limits unless you explicitly select a genuine upgrade; new buyers receive the complete latest plan.
        </div>
        <div className="grid gap-4 p-6 xl:grid-cols-3">
          {data.plans.map((plan) => <PlanForm key={`${plan.planId}-${plan.updatedAt ?? 'default'}`} plan={plan} />)}
        </div>
      </AdminPanel>
    ...
    <form onSubmit={submit} className="rounded-xl border border-[rgba(255,255,255,0.09)] bg-[#24211e] p-5">
      ...
      {plan.planId !== 'free' && (
        <p className="mt-2 text-xs leading-5 text-[#aaa59d]">
          Select any changed limit you want evaluated for active existing subscribers. It is applied only where the new value is an upgrade for that subscriber; equal values and downgrades are always preserved.
        </p>
      )}
      <div className="mt-4 space-y-3">
        {fields.map(([key, label, maximum]) => {
          ...
          return (
            <div key={key} className="rounded-lg border border-white/8 bg-[#1c1a18] p-3">
              ...
              />
              {plan.planId !== 'free' && (
                <label className={`mt-2 flex items-start gap-2 text-xs ${canPropagate ? 'cursor-pointer text-[#d7d2ca]' : 'text-[#817c75]'}`}>
                  <input
                    type="checkbox"
                    disabled={!canPropagate}
                    checked={propagateLimitFields.includes(key)}
                    onChange={(event) => setPropagateLimitFields((current) =>
      ...
      <button type="submit" disabled={update.isPending} className="admin-primary-button mt-5 w-full">
        {update.isPending ? <><LoaderCircle size={15} className="animate-spin" /> Saving…</> : `Save ${plan.planId} plan`}
      </button>
    </form>
  );
}
...
function PlanNumberField({ label, value, maximum, onChange }: { label: string; value: number; maximum: number; onChange: (value: number) => void }) {
  return <label className="block"><span className="mb-1 block text-[10px] uppercase tracking-wide text-[#aaa59d]">{label}</span><AdminNumberInput required min={0} max={maximum} value={value} onValueChange={onChange} className="w-full rounded-lg border border-[rgba(255,255,255,0.12)] bg-[#1c1a18] px-3 py-2 text-sm outline-none focus:border-[#e8816a]" /></label>;
}
...
function LimitChangeBadge({ change }: { change: LimitChange }) {
  const className = change === 'upgrade'
    ? 'border-[#52c58c]/40 bg-[#52c58c]/10 text-[#52c58c]'
    : change === 'downgrade'
      ? 'border-[#f0a842]/40 bg-[#f0a842]/10 text-[#f0a842]'
      : 'border-white/10 text-[#817c75]';
  return <span className={`rounded border px-1.5 py-0.5 text-[9px] font-bold uppercase ${className}`}>{change}</span>;
}
