...
export const useUpdateAdminSettings = () => {
  ...
  return useMutation({
    mutationFn: async ({ settings, actionPassword, changeReason }: UpdateAdminSettingsInput) =>
      (
        await api.put<ApiEnvelope<AdminSettings>>(ADMIN_SETTINGS_ENDPOINTS.update, settings, {
          headers: {
    ...
    onMutate: () => ({
      toastId: toast.loading('Saving settings…', 'Applying the reviewed platform policy.'),
    }),
