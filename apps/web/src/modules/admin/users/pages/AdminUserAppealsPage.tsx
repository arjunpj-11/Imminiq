...
export default function AdminUserAppealsPage() {
  ...
  return (
    <main className="mx-auto max-w-310 px-5 py-8 sm:px-8">
      ...
      {query.isLoading ? (
        <div className="mt-7">
          <AdminCardSkeleton cards={4} label="Loading account appeal metrics" />
        </div>
      ) : (
        <AdminMetricGrid
          metrics={[
        ...
        toolbar={
          <div className="flex flex-wrap gap-3">
            <select
              ...
              aria-label="Filter account appeals by status"
              value={status}
              onChange={(event) => {
          ...
          <>
            <div className="admin-table-scroll overflow-x-auto">
              <table className="admin-table w-full min-w-250 text-left text-sm">
                <caption className="sr-only">User restriction appeals</caption>
                <thead>
                  <tr>
                    <th scope="col">Case</th>
                    <th scope="col">User</th>
                    <th scope="col">Submitted</th>
                    <th scope="col">Restriction reason</th>
                ...
                <tbody>
                  {data.items.map((appeal) => (
                    <tr key={appeal.id}>
                      ...
                      <td>
                        <Link
                          className="font-semibold text-[#e8816a]"
                          to={ADMIN_USERS_ROUTES.detail(appeal.userId)}
                        >
                          {appeal.userName}
                        </Link>
...
function AppealDecisionDialog({
  ...
  return (
    <Modal
      ...
      ariaLabel="Review account appeal"
    ...
    >
      ...
      <label className="admin-field mt-4 block">
        <span>User-facing review note</span>
        <textarea
          rows={5}
          maxLength={2000}
          value={reviewNote}
          onChange={(event) => setReviewNote(event.target.value)}
          placeholder="Explain the review and the next step…"
        />
      </label>
      <label className="mt-4 flex items-center gap-3 text-sm text-[#aaa59d]">
        <input
          type="checkbox"
          checked={notifyEmail}
