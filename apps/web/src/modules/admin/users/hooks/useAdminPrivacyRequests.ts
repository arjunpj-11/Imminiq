...
export const useAdminPrivacyRequests = (query: AdminPrivacyRequestsQuery) => useQuery({
  queryKey: adminUsersKeys.privacyRequestList(query),
  queryFn: async () => (await api.get<ApiEnvelope<AdminPrivacyRequestsData>>(ADMIN_USERS_ENDPOINTS.privacyRequests, { params: query })).data.data,
