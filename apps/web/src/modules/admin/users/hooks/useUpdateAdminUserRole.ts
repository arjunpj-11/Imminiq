...
export const useUpdateAdminUserRole = (userId: string) => {
  ...
  return useMutation({
    mutationFn: ({ role, reason, mfaCode }: { role: 'user' | 'moderator' | 'admin'; reason: string; mfaCode?: string }) =>
      api.patch(ADMIN_USERS_ENDPOINTS.role(userId), { role, reason }, { headers: mfaCode ? { 'X-Admin-MFA-Code': mfaCode } : undefined }),
    onSuccess: async () => {
