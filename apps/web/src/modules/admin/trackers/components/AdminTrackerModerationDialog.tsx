...
import type { AdminTracker, AdminTrackerLifecyclePayload } from '../types/admin-trackers.types';
...
export default function AdminTrackerModerationDialog({
  ...
  const [reasonCode, setReasonCode] = useState<AdminTrackerLifecyclePayload['reasonCode']>(
  ...
  return (
    <Modal open={Boolean(tracker)} onClose={onClose} preventClose={mutation.isPending} ariaLabel={`${label} tracker`} contentClassName="max-w-xl bg-[#1c1a18] text-[#f2f0eb]">
      <h2 className="font-editorial text-2xl font-bold">{label} {tracker?.title ?? 'tracker'}?</h2>
      ...
      <label className="admin-field mt-5 block"><span>Reason category</span><select value={reasonCode} onChange={(event) => setReasonCode(event.target.value as AdminTrackerLifecyclePayload['reasonCode'])}><option value="incorrect_content">Incorrect or misleading content</option><option value="unsafe_content">Unsafe or offensive content</option><option value="copyright">Copyright concern</option><option value="spam_or_abuse">Spam or abuse</option><option value="broken_learning_path">Broken learning path</option><option value="owner_request">Owner request</option><option value="appeal_accepted">Appeal accepted</option><option value="other">Other</option></select></label>
