    ...
    >
      ...
      <label className="admin-field mt-5 block">
        ...
        <select
          value={reasonCode}
          onChange={(event) =>
            setReasonCode(event.target.value as AdminTrackerLifecyclePayload['reasonCode'])
          }
        >
          <option value="incorrect_content">Incorrect or misleading content</option>
      ...
      <label className="mt-4 flex items-start gap-3 text-sm text-[#aaa59d]">
        <input
          ...
          className="mt-1"
        />
        Queue an email with this explanation. An in-app notification is always sent.
      </label>
