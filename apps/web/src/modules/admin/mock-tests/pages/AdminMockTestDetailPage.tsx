...
export default function AdminMockTestDetailPage() {
  ...
  const [moderating, setModerating] = useState<AdminMockTestLifecyclePayload['action'] | null>(
    null
  );
  const [learnerPreview, setLearnerPreview] = useState(false);
  const [restoreQuestion, setRestoreQuestion] = useState<AdminMockTestQuestion | null>(null);
    ...
    <main className="mx-auto max-w-262.5 px-5 py-8 sm:px-8">
              ...
              >
                ...
                {question.moderationReason && (
                  <p className="mt-3 rounded-lg border border-[#f0a842]/20 bg-[#f0a842]/5 p-3 text-xs text-[#f0c060]">
                    <strong>Moderation note:</strong> {question.moderationReason}
                  </p>
                )}
      ...
      <AdminPanel title="Moderation history">
        {!data.moderationHistory.length ? (
          <AdminEmpty>No administrative changes have been recorded for this test.</AdminEmpty>
        ) : (
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6 text-[#aaa59d]">
          This restores the question in the Question Bank and every linked mock test. Tests
          automatically suspended because they became empty will be reactivated.
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button className="admin-button" onClick={closeRestore}>
            Cancel
          </button>
          <button
