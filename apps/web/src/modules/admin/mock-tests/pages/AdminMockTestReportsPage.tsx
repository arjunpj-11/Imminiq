...
export default function AdminMockTestReportsPage() {
  ...
  const { data, dataUpdatedAt, isLoading, isFetching, isPlaceholderData, isError, error, refetch } =
    useAdminMockTestReports({ status, page });
          ...
          <>
            <div className="admin-table-scroll overflow-x-auto">
              <table className="admin-table w-full min-w-250 text-left text-sm">
                <caption className="sr-only">Mock test moderation reports</caption>
                <thead>
                  <tr>
                    <th scope="col">Age</th>
...
function ReportReviewDialog({
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="admin-field">
          ...
          <select
            value={status}
            onChange={(event) => {
              const next = event.target.value as AdminMockTestIssueUpdatePayload['status'];
              setStatus(next);
        ...
        <label className="admin-field">
          ...
          <select
            ...
            onChange={(event) =>
              setResolutionAction(
                event.target.value as AdminMockTestIssueUpdatePayload['resolutionAction']
              )
            }
          >
            <option value="none">No linked content action</option>
            <option value="question_corrected">Correct the live question</option>
      ...
      {resolutionAction === 'question_corrected' && status !== 'reviewing' && (
        <div className="mt-4 grid gap-4 rounded-xl border border-[#e8816a]/30 bg-[#e8816a]/5 p-4">
          <p className="text-sm text-[#d9b4a8]">
            This updates the question shown to learners while preserving completed attempt scores.
          </p>
          <label className="admin-field">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="admin-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setCorrectedDifficulty(event.target.value as 'easy' | 'medium' | 'hard')
                }
              >
            ...
            <label className="admin-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setCorrectedPoints(Math.max(1, Math.min(100, Number(event.target.value) || 1)))
                }
              />
            </label>
          </div>
      ...
      {status !== 'reviewing' &&
        ['question_disabled', 'test_suspended', 'test_deleted'].includes(resolutionAction) && (
      ...
      <div className="mt-6 flex flex-wrap justify-between gap-2">
        {issue && (
          <Link to={ADMIN_MOCK_TESTS_ROUTES.detail(issue.testId)} className="admin-button">
            Open full test
          </Link>
        )}
