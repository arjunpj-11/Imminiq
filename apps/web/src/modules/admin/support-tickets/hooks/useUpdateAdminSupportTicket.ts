...
export const useUpdateAdminSupportTicket = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      api.patch(
        ADMIN_SUPPORT_TICKETS_ENDPOINTS.update(id),
        {
          status,
          resolutionNote,
          notificationMessage,
    ...
    onSuccess: async (_data, _input, context) => {
      toast.update(context.toastId, {
        ...
        description: 'The requester was notified and the audit trail was updated.',
        tone: 'success',
      });
    ...
    onError: (error, _input, context) => {
      ...
      toast.update(context.toastId, {
        ...
        description: getUserFacingError(error, 'The ticket and notification could not be saved.'),
        tone: 'error',
        duration: 5600,
      });
    },
