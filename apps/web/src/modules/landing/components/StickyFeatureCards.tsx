...
const toneClasses = {
  ...
  green: 'text-[#2d6a47] dark:text-[#5cc98a] bg-[rgba(45,106,71,0.08)] border-[rgba(45,106,71,0.18)] dark:bg-[rgba(92,201,138,0.10)] dark:border-[rgba(92,201,138,0.24)]',
  amber: 'text-[#8a6200] dark:text-[#f0a842] bg-[rgba(138,98,0,0.08)] border-[rgba(138,98,0,0.18)] dark:bg-[rgba(240,168,66,0.10)] dark:border-[rgba(240,168,66,0.24)]',
}
    ...
    <section className="bg-[#f5ede4] px-4 py-16 text-[#1a1714] dark:bg-[#141412] dark:text-[#f2f0eb] sm:px-8 lg:px-10">
      <div className="mx-auto max-w-340">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="font-['DM_Mono',monospace] text-[10px] uppercase tracking-[0.18em] text-[#b84c2b] dark:text-[#e8816a]">Product stack</p>
          ...
          <p className="max-w-94 text-[14px] leading-[1.7] text-[#6b5f58] dark:text-[#9b9a92]">
            Sticky cards stack vertically like your reference design, but the content is fully Imminiq-focused and launch-safe.
          </p>
        </div>
              ...
              <div className="flex flex-[0.48] flex-col justify-between gap-8 pt-10 md:pt-0">
                  ...
                  <div className="grid h-full grid-cols-5 gap-2 opacity-80">
                    {Array.from({ length: 20 }).map((_, itemIndex) => (
                      <span
                        ...
                        className={cn(
                          'rounded-xl border',
                          itemIndex % 3 === 0 ? toneClasses[card.tone as keyof typeof toneClasses] : 'border-white/8 bg-white/5'
                        )}
                      />
                    ))}
                  </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {card.items.map((item) => (
                    <span
                      key={item}
                      className={cn("rounded-full border px-4 py-2 font-['DM_Mono',monospace] text-[9px] uppercase tracking-[0.12em]", toneClasses[card.tone as keyof typeof toneClasses])}
                    >
                      {item}
                    </span>
                  ))}
