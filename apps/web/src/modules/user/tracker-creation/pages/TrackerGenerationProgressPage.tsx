...
import {
  TrackerCreationActivityChips,
  TrackerCreationProgressStatusCard,
  TrackerCreationWorkflowFooter,
} from '../components/TrackerCreationWorkflowStatus';
import {
  TrackerCreationContextCard,
  TrackerCreationErrorBanner,
  TrackerCreationStepCard,
  TrackerCreationWorkflowHeader,
  TrackerCreationWorkflowHero,
} from '../components/TrackerCreationWorkflowLayout';
import { generationSteps } from '../constants/tracker-creation.constants';
import { useRoadmapJobStatus } from '../hooks/useRoadmapJobStatus';
import { useTrackerCreationStore } from '../store/useTrackerCreationStore';
import type {
  ActivityChip,
  IJobStatusApiData,
  ...
  JobTerminalState,
  ProgressStepState,
} from '../types/tracker-creation.types';
import { clampProgress, normalizeProgressStepIndex } from '../utils/tracker-creation-progress';
...
export default function TrackerGenerationProgressPage() {
  const navigate = useNavigate();
  const { jobId } = useParams<{ jobId: string }>();
  const setActiveRoadmapJobId = useTrackerCreationStore((state) => state.setActiveRoadmapJobId);
  const clearIntake = useTrackerCreationStore((state) => state.clearIntake);
  const topic = useTrackerCreationStore(
    (state) => state.step1Data?.topic || 'MERN stack interviews'
  );
  const goal = useTrackerCreationStore(
    (state) => state.step1Data?.goal || 'Crack top company interviews'
  );
  const level = useTrackerCreationStore((state) => state.step2Data?.level || 'intermediate');
  ...
  return (
    <div className="flex min-h-screen flex-col bg-(--surface-canvas) font-[DM_Sans,sans-serif] text-(--text-primary)">
      <TrackerCreationWorkflowHeader label="AI Roadmap Generator" />
      ...
      <main className="mx-auto flex w-full max-w-280 flex-1 flex-col gap-5 px-4 py-6 sm:px-6 sm:py-8 md:px-12 md:py-10">
        ...
        {statusError ? <TrackerCreationErrorBanner message={statusError} /> : null}
