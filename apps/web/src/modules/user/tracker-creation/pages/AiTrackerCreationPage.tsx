...
import { useSaveTrackerGoal } from '../hooks/useSaveTrackerGoal';
import { useSaveTrackerLevel } from '../hooks/useSaveTrackerLevel';
...
export default function AiTrackerCreationPage() {
  const navigate = useNavigate();
  const storedMessages = useTrackerCreationStore((state) => state.intakeMessages);
  const storedProfile = useTrackerCreationStore((state) => state.intakeProfile);
  const saveIntake = useTrackerCreationStore((state) => state.saveIntake);
  ...
  const saveStepOne = useSaveTrackerGoal();
  const saveStepTwo = useSaveTrackerLevel();
      ...
      <header className="shrink-0 border-b border-(--border-subtle) bg-(--surface-canvas)/92 backdrop-blur-xl dark:border-white/15">
        <div className="mx-auto flex h-16 w-full max-w-300 items-center justify-between gap-4 px-4 sm:px-6 lg:px-10">
          <div className="flex items-center gap-3">
            ...
            <TrackerCreationBrandLink hideWordmarkOnMobile />
          </div>
