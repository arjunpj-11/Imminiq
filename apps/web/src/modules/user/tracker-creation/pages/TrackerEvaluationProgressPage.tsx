...
import { evaluationSteps } from '../constants/tracker-creation.constants';
import {
...
export default function TrackerEvaluationProgressPage() {
  const navigate = useNavigate();
  const { jobId } = useParams<{ jobId: string }>();
  const roadmapTopic = useTrackerCreationStore(
    (state) => state.step1Data?.topic || 'MERN stack interviews'
  );
  const roadmapGoal = useTrackerCreationStore(
  ...
  return (
    <div className="flex min-h-screen flex-col bg-(--surface-canvas) font-[DM_Sans,sans-serif] text-(--text-primary)">
      <TrackerCreationWorkflowHeader label="AI Roadmap Evaluation" />
      ...
      <main className="mx-auto flex w-full max-w-280 flex-1 flex-col gap-5 px-4 py-6 sm:px-6 sm:py-8 md:px-12 md:py-10">
        ...
        {statusError || (!isLoading && !data) ? (
          statusError ? (
            <TrackerCreationErrorBanner message={statusError} />
          ) : null
        ) : null}
