...
import type { ProgressStepState } from '../types/tracker-creation.types';
import TrackerCreationBrandLink from './TrackerCreationBrandLink';

interface ITrackerCreationWorkflowHeaderProps {
  label?: string;
  actions?: ReactNode;
}

export function TrackerCreationWorkflowHeader({
  label,
  actions,
}: ITrackerCreationWorkflowHeaderProps) {
  return (
...
interface ITrackerCreationWorkflowHeroProps {
  eyebrow: string;
  title: string;
  description: string;
}

export function TrackerCreationWorkflowHero({
  eyebrow,
  title,
  description,
}: ITrackerCreationWorkflowHeroProps) {
  return (
...
export interface ITrackerCreationContextRow {
  label: string;
  value: string;
}

interface ITrackerCreationContextCardProps {
  title: string;
  description: string;
  rows: ITrackerCreationContextRow[];
}

export function TrackerCreationContextCard({
  title,
  description,
  rows,
}: ITrackerCreationContextCardProps) {
  return (
...
interface ITrackerCreationStepCardProps {
  title: string;
  description: string;
...
export function TrackerCreationStepCard({
  title,
  description,
  steps,
  states,
}: ITrackerCreationStepCardProps) {
  return (
    <article className="h-full rounded-2xl border border-(--border-subtle) bg-(--surface-card) p-5 shadow-[0_12px_36px_rgba(26,23,20,0.06)] dark:border-white/15 dark:bg-(--surface-card) sm:p-6">
      <div className="mb-5">
        <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-(--brand-500)">
...
interface ITrackerCreationErrorBannerProps {
  message: string;
}

export function TrackerCreationErrorBanner({ message }: ITrackerCreationErrorBannerProps) {
  return (
    <div
