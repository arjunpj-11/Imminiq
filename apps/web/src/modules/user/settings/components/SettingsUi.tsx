    ...
    <div className="flex items-center justify-between gap-5 border-t border-(--border-subtle) py-4 first:border-t-0 dark:border-(--border-subtle)">
      ...
      <button
        ...
        className={cn(
          ...
          checked ? 'bg-(--brand-500) dark:bg-(--brand-500)' : 'bg-[#d5c8be] dark:bg-[#3a3530]',
...
export function UnsavedChangesDialog({
  ...
  return (
    ...
    >
      ...
      >
        You changed settings on this page. You can save them now, discard them and leave, or stay on
        this page.
      </p>

      <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        <Button variant="outline-danger" disabled={isSaving} onClick={onDiscard}>
          Discard and Leave
        </Button>
        <Button variant="secondary" disabled={isSaving} onClick={onStay}>
          Stay
        </Button>
        <Button loading={isSaving} loadingText="Saving..." onClick={onSaveChanges}>
          Save Changes
        </Button>
      </div>
    </Modal>
