...
export default function TrackerClanBattlePage() {
  ...
  const busy = chooseCheckpoint.isPending || answerNode.isPending || usePower.isPending || quitChallenge.isPending;
  const error = chooseCheckpoint.error || answerNode.error || usePower.error || quitChallenge.error;
  const timerClock = challenge?.status === 'completed' && challenge.completedAt
    ? new Date(challenge.completedAt).getTime()
    : clock;
  const secondsLeft = Math.max(0, Math.ceil(((challenge?.endsAt ? new Date(challenge.endsAt).getTime() : timerClock) - timerClock) / 1000));
    ...
    <AppShellBoundary>
      <main className="mx-auto flex w-full max-w-300 flex-1 flex-col gap-6 px-4 py-5 pb-20 sm:px-7">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2"><span className={cn('rounded-full px-4 py-2 font-mono text-[9px] font-black uppercase tracking-[.16em] text-white', finished ? 'bg-(--text-secondary)' : 'bg-(--brand-500)')}>{finished ? 'Battle ended' : 'Live challenge'}</span>{challenge.canQuit && <button type="button" onClick={() => setQuitDialogOpen(true)} className="rounded-full border border-red-500/30 px-4 py-2 text-[10px] font-extrabold text-red-500 hover:bg-red-500/8">Quit battle</button>}</div>
        </div>
        ...
        <section className="grid items-center gap-5 rounded-2xl border border-(--border-subtle) bg-(--surface-card) px-5 py-6 shadow-sm md:grid-cols-[1fr_auto_1fr] dark:border-white/15">
          ...
          <div className="text-center"><p className="font-mono text-[9px] uppercase tracking-[.18em] text-(--text-secondary)">{finished ? 'Final time remaining' : 'Guild node race'}</p><strong className={cn('mt-1 block font-serif text-4xl', !finished && secondsLeft < 60 ? 'text-red-500' : 'text-(--brand-500)')}>{timer}</strong></div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          <button type="button" disabled={busy || challenge.pushBackPowers < 1 || finished} onClick={() => usePower.mutate({ trackerId, challengeId }, { onSuccess: () => { setFeedback('Power used — your opponent moved back two nodes.'); } })} className="rounded-full border border-(--border-subtle) bg-(--surface-card) px-5 py-2.5 text-[11px] font-extrabold shadow-sm disabled:opacity-40 dark:border-white/15">⚡ Push back ×2 <span className="ml-2 text-(--brand-500)">{challenge.pushBackPowers}</span></button>
        </div>

        <section aria-label="Battle rules" className="grid gap-2 sm:grid-cols-3">
          <div className="rounded-xl border border-(--border-subtle) bg-(--surface-card) px-4 py-3 text-center text-[11px] font-bold dark:border-white/15"><span className="text-emerald-600">Correct</span> · move forward 1</div>
          <div className="rounded-xl border border-(--border-subtle) bg-(--surface-card) px-4 py-3 text-center text-[11px] font-bold dark:border-white/15"><span className="text-red-500">Incorrect</span> · move back 1, never below start</div>
          <div className="rounded-xl border border-[#d6ad47]/40 bg-[#f4c95d]/10 px-4 py-3 text-center text-[11px] font-bold"><span className="text-[#9a7210] dark:text-[#f4c95d]">Checkpoint</span> · one try; miss moves back 3</div>
        </section>
        ...
        {finished ? (
          <section className="rounded-2xl border border-[#d6ad47]/40 bg-[#f4c95d]/10 p-8 text-center"><div className="text-5xl">{challenge.winnerId ? '🏆' : '🤝'}</div><h2 className="mt-3 font-serif text-3xl font-extrabold">{challenge.quitById === currentUserId ? 'You left the battle' : challenge.quitById ? `${rival!.name} left — you win!` : challenge.winnerId ? (won ? 'You won the node race!' : `${rival!.name} won the race`) : 'The battle ended in a draw'}</h2><p className="mt-2 text-sm text-(--text-secondary)">{challenge.viewerScore} – {challenge.opponentLiveScore} correct answers</p><button type="button" onClick={() => setHistoryOpen(true)} className="mt-5 rounded-xl bg-[#171512] px-6 py-3 text-xs font-extrabold text-white dark:bg-[#f2f0eb] dark:text-[#171512]">View question history</button></section>
        ) : challenge.checkpointDecisionRequired ? (
          <section className="mx-auto w-full max-w-190 rounded-2xl border border-[#d6ad47]/45 bg-(--surface-card) p-7 text-center shadow-xl"><div className="text-4xl">★</div><p className="mt-3 font-mono text-[9px] font-black uppercase tracking-[.18em] text-[#9a7210] dark:text-[#f4c95d]">Hard checkpoint</p><h2 className="mt-2 font-serif text-2xl font-extrabold">Attempt before revealing the question?</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-relaxed text-(--text-secondary)">You get one attempt. Solve it to earn a power that sends your opponent back two nodes; a wrong answer sends you back three. You may skip without seeing it.</p><div className="mt-6 flex justify-center gap-3"><button disabled={busy} onClick={() => chooseCheckpoint.mutate({ trackerId, challengeId, decision: 'skip' })} className="rounded-lg border border-(--border-subtle) px-5 py-3 text-xs font-bold">Skip checkpoint</button><button disabled={busy} onClick={() => chooseCheckpoint.mutate({ trackerId, challengeId, decision: 'attempt' })} className="rounded-lg bg-(--brand-500) px-6 py-3 text-xs font-extrabold text-white">Attempt for power</button></div></section>
        ) : question ? (
          ...
          <section className="mx-auto w-full max-w-190 rounded-2xl border border-(--border-subtle) bg-(--surface-card) p-8 text-center shadow-xl"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-(--brand-500)/25 border-t-(--brand-500)" /><h2 className="mt-4 font-serif text-xl font-extrabold">Preparing more questions…</h2><p className="mt-2 text-sm text-(--text-secondary)">Your position is safe. The battle will continue as soon as the next question arrives.</p></section>
        )}

        {!finished && (feedback || error) && <div className={cn('fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full px-5 py-3 text-xs font-bold text-white shadow-2xl', error || challenge.lastAnswerCorrect === false ? 'bg-red-600' : 'bg-[#34714e]')}>{error ? getUserFacingError(error, 'The arena could not process that move.') : feedback}</div>}
      </main>
      <ConfirmDialog
        open={quitDialogOpen}
        ...
        onClose={() => { if (!quitChallenge.isPending) setQuitDialogOpen(false); }}
        onConfirm={() => quitChallenge.mutate(
          { trackerId, challengeId },
          { onSuccess: () => setQuitDialogOpen(false) }
        )}
      />
      <Modal open={historyOpen} onClose={() => setHistoryOpen(false)} ariaLabel="Battle question history" contentClassName="max-h-[90vh] max-w-5xl overflow-y-auto p-0">
        <header className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-3 border-b border-(--border-subtle) bg-(--surface-elevated) px-5 py-4">
          <div><p className="font-mono text-[8px] font-black uppercase tracking-[.16em] text-(--brand-500)">Study review</p><h2 className="font-serif text-2xl font-extrabold">Battle question history</h2></div>
          <div className="flex gap-2"><button type="button" disabled={!historyQuery.data || pdfDownloading} onClick={() => { if (!historyQuery.data) return; setPdfError(null); setPdfDownloading(true); void downloadBattleHistoryPdf(historyQuery.data).catch(() => setPdfError('The PDF could not be created. Please try again.')).finally(() => setPdfDownloading(false)); }} className="rounded-lg bg-(--brand-500) px-4 py-2.5 text-xs font-extrabold text-white disabled:opacity-50">{pdfDownloading ? 'Creating PDF…' : 'Download PDF'}</button><button type="button" onClick={() => setHistoryOpen(false)} className="rounded-lg border border-(--border-subtle) px-4 py-2.5 text-xs font-bold">Close</button></div>
        </header>
        <div className="p-5">
          {historyQuery.isLoading && <div className="py-16 text-center text-sm text-(--text-secondary)">Loading both players’ answers…</div>}
          {historyQuery.error && <div className="rounded-xl bg-red-500/10 p-4 text-sm font-semibold text-red-600">{getUserFacingError(historyQuery.error, 'Unable to load battle history.')}</div>}
          {pdfError && <div className="mb-4 rounded-xl bg-red-500/10 p-4 text-sm font-semibold text-red-600">{pdfError}</div>}
          {historyQuery.data && (historyQuestions.length === 0 ? <p className="rounded-xl border border-dashed border-(--border-subtle) p-8 text-center text-sm text-(--text-secondary)">No questions were asked in this battle.</p> : <div className="space-y-4">{historyQuestions.map((question, index) => <article key={question.questionId} className="overflow-hidden rounded-xl border border-(--border-subtle) bg-(--surface-card)"><div className="p-5"><p className="font-mono text-[8px] font-black uppercase tracking-wider text-(--text-secondary)">Question {index + 1} · {question.topicTitle}{question.isCheckpoint ? ' · Checkpoint' : ''}</p><p className="mt-2 text-sm font-bold leading-relaxed">{question.prompt}</p><div className="mt-4 rounded-lg bg-emerald-500/8 px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-wide text-emerald-700 dark:text-emerald-400">Correct answer</p><p className="mt-1 text-sm font-extrabold text-emerald-800 dark:text-emerald-300">{question.correctAnswer}</p></div></div><div className="border-t border-(--border-subtle) bg-(--surface-canvas) p-5"><p className="mb-3 font-mono text-[8px] font-black uppercase tracking-wider text-(--text-secondary)">Attempts</p><div className="grid gap-3 sm:grid-cols-2">{question.attempts.map(({ player, answer }) => <div key={player.userId} className="rounded-lg border border-(--border-subtle) bg-(--surface-card) p-4"><div className="flex items-center justify-between gap-3"><p className="truncate text-xs font-extrabold">{player.name}</p>{answer && <span className={cn('text-[10px] font-black', answer.isCorrect ? 'text-emerald-600' : 'text-red-500')}>{answer.isCorrect ? 'Correct' : 'Incorrect'}</span>}</div><p className={cn('mt-2 text-sm font-semibold', !answer && 'italic text-(--text-secondary)')}>{answer?.answer ?? 'Not asked'}</p></div>)}</div></div></article>)}</div>)}
        </div>
      </Modal>
    </AppShellBoundary>
  );
