...
export default function TrackerCard({
  ...
  const isUnavailable = Boolean(
    tracker.moderationStatus && tracker.moderationStatus !== "active",
  );
      ...
      <article className="render-lazy group relative flex min-h-[430px] flex-col overflow-visible rounded-2xl border-[1.5px] border-(--border-subtle) bg-(--surface-card) p-5 shadow-(--shadow-1) transition duration-200 hover:-translate-y-1 hover:border-[rgba(184,76,43,0.24)] hover:shadow-(--shadow-2) dark:hover:border-[rgba(232,129,106,0.26)]">
        ...
        <header className="relative flex items-start justify-between gap-3">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            <span
              className={cn(
                "rounded-full border px-2.5 py-1 text-[10.5px] font-bold",
                tone.badge,
              )}
            >
              {statusLabel}
          ...
          <div className="relative flex shrink-0 items-center gap-1">
            {(cloneSource ||
              ...
              <button
                ...
                onClick={() =>
                  navigate(
                    ROUTES.trackerClan(cloneSource?.trackerId ?? tracker._id),
                  )
                }
        ...
        <section
          className={cn(
            "relative mt-5 rounded-xl border border-(--border-subtle) p-4",
            tone.soft,
          )}
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-(--text-secondary)">
                ...
                {isArchived
                  ? "Archived tracker"
                  : formatRelativeTime(tracker.lastActiveAt)}
              </div>
          ...
          <div className="mt-3 flex min-h-8 items-center justify-between gap-3 border-t border-(--border-subtle) pt-3">
            <div className="inline-flex min-w-0 items-center gap-1.5 text-[11.5px] font-semibold text-(--text-secondary)">
              {tracker.status === "completed" ? (
                ...
                <span
                  className={cn(
                    "h-2 w-2 shrink-0 rounded-full bg-linear-to-r",
                    tone.bar,
                  )}
                />
              )}
          ...
          <div className="space-y-2">
            ...
            {deleteConfirmationStep === 2 && (
              <p className="font-semibold text-red-600 dark:text-red-400">
                Community users will no longer be able to view this published
                tracker.
              </p>
            )}
            {deleteError && (
