...
const DeleteIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path
      d="M4 7H20M9 7V4H15V7M7 7L8 20H16L17 7M10 11V16M14 11V16"
      stroke="currentColor"
      strokeWidth="1.6"
      strokeLinecap="round"
      ...
      >
        ...
        <div className="mb-5 flex items-start justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={cn(
                'rounded-full border px-3 py-1 font-mono text-[8px] uppercase tracking-[0.12em]',
                tone.badge
              )}
            >
          ...
          <div ref={menuRef} className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-10 z-30 w-52 overflow-hidden rounded-md border-[1.5px] border-(--border-subtle) bg-(--surface-card) shadow-[0_16px_56px_rgba(26,23,20,0.18)] dark:border-(--border-subtle) dark:bg-(--surface-card)">
                <button
                  type="button"
                  onClick={(e) => handleMenuAction(e, () => onInfo(tracker._id))}
                  className="flex w-full items-center gap-2.5 px-4 py-3 text-left text-[13px] font-semibold text-(--text-primary) transition hover:bg-[rgba(184,76,43,0.08)] hover:text-(--brand-500) dark:text-(--text-primary) dark:hover:bg-[rgba(232,129,106,0.10)] dark:hover:text-(--brand-500)"
                >
                  <InfoIcon />
                  Info / Manage
                </button>
                <button
                  type="button"
                  onClick={(e) => handleMenuAction(e, () => onQuickRevision(tracker._id))}
                  className="flex w-full items-center gap-2.5 px-4 py-3 text-left text-[13px] font-semibold text-(--text-primary) transition hover:bg-[rgba(184,76,43,0.08)] hover:text-(--brand-500) dark:text-(--text-primary) dark:hover:bg-[rgba(232,129,106,0.10)] dark:hover:text-(--brand-500)"
                >
                  <QuickRevisionIcon />
                  Quick Revision
                ...
                {onArchive && !isSharedCoOwner && (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={(e) => handleMenuAction(e, () => onArchive(tracker._id))}
                      className={cn(
                        'flex w-full items-center gap-2.5 px-4 py-3 text-left text-[13px] font-semibold transition',
                        isArchived
                          ? 'text-(--success) hover:bg-[rgba(45,106,71,0.08)] dark:text-(--success) dark:hover:bg-[rgba(92,201,138,0.10)]'
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
          ...
          {isUnavailable ? (
            ...
            <button
              ...
              className="rounded-sm border-[1.5px] border-(--border-subtle) px-3.5 py-2 text-[12px] font-bold text-(--text-secondary) transition hover:border-(--brand-500) hover:bg-[rgba(184,76,43,0.08)] hover:text-(--brand-500) dark:border-(--border-subtle) dark:text-(--text-secondary) dark:hover:text-(--brand-500)"
            >
              View Published
            </button>
          ) : isSharedCoOwner ? (
          ...
          <div className="space-y-2">
            <p>
              {deleteConfirmationStep === 2 ? (
                <>
                  “{tracker.title}” is published. Deleting it will permanently remove it from your
                  trackers and from Community.
                </>
              ) : (
                <>
                  Are you sure you want to delete “{tracker.title}”? Your roadmap and learning
                  progress will no longer be available.
                </>
              )}
            </p>
            {deleteConfirmationStep === 2 && (
