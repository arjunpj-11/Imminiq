  ...
  if (eventType === 'result') {
    return (
      <article className="mx-auto max-w-xl rounded-xl border border-[#d6ad47]/35 bg-[linear-gradient(135deg,rgba(244,201,93,.14),rgba(184,76,43,.07))] px-5 py-4 text-center shadow-sm">
        <p className="font-mono text-[8px] font-bold uppercase tracking-[.16em] text-[#9a7210] dark:text-[#f4c95d]">⚔ Battle result</p>
        ...
        <p className="mt-1 text-sm font-extrabold">
          {challenge.challenger.name} {challenge.challengerScore ?? 0} – {challenge.opponentScore ?? 0} {challenge.opponent?.name}
        </p>
        <p className="mt-1 text-[10px] text-(--text-secondary)">Same tracker contest · {challenge.durationMinutes} minute limit</p>
      </article>
    );
  }
  ...
  return (
    <div className={cn('flex gap-3', mine && 'flex-row-reverse')}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {challenge.canAccept && (
          <button type="button" disabled={busy} onClick={onAccept} className="rounded-md bg-[#171512] px-4 py-2 text-[11px] font-extrabold text-white disabled:opacity-50 dark:bg-[#f2f0eb] dark:text-[#171512]">
            Accept & start
          </button>
        )}
        ...
        {challenge.canCancel && (
          <button type="button" disabled={busy} onClick={onCancel} className={cn('rounded-md px-4 py-2 text-[11px] font-bold text-red-500 hover:bg-red-500/8', busy && 'opacity-50')}>
            Cancel
          </button>
        )}
      </div>
