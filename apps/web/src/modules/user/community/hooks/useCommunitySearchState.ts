...
const VALID_SORTS = new Set<CommunitySort>(['top-rated', 'most-cloned', 'newest']);
...
export function useCommunitySearchState() {
  ...
  const state = useMemo(() => {
    ...
    return {
      ...
      sort: sortParam && VALID_SORTS.has(sortParam) ? sortParam : 'top-rated',
      page: positiveInteger(searchParams.get('page')),
    };
  }, [searchParams]);

  const update = useCallback(
    (patch: Partial<typeof state>, options: { resetPage?: boolean; replace?: boolean } = {}) => {
      ...
      setOrDelete('topics', merged.selectedTopics.join(','), merged.selectedTopics.length === 0);
      setOrDelete('rating', String(merged.minRating), merged.minRating === null);
  ...
  return {
    ...
    setMinRating: (minRating: number | null) => update({ minRating }, { resetPage: true }),
    setVerifiedOnly: (verifiedOnly: boolean) => update({ verifiedOnly }, { resetPage: true }),
