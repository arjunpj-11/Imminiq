...
export default function LeaderboardMyRankBar({ entry }: { entry: ILeaderboardCurrentUser }) {
  return (
    <section
      className="flex items-center gap-4 rounded-2xl border-[1.5px] border-[rgba(184,76,43,0.2)] bg-(--surface-card) px-5 py-4 dark:border-[rgba(232,129,106,0.2)] dark:bg-(--surface-card)"
    ...
    >
      <div className="w-13 shrink-0">
        ...
        <div className="font-ui text-[28px] font-black leading-none text-(--brand-500) tabular-nums dark:text-(--brand-500)">
          {entry.rank}
        </div>
      </div>
      ...
      <div className="hidden shrink-0 text-right min-[480px]:block">
        <div className="font-mono text-[14px] font-bold text-(--text-primary) tabular-nums dark:text-(--text-primary)">
          {formatNumber(entry.totalXp)}
        </div>
        <div className="mt-px text-[9.5px] uppercase tracking-wider text-[#b0a097] dark:text-[#6b6460]">
          Total XP
        </div>
      </div>
