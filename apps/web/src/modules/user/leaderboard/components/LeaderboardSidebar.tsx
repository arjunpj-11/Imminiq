...
export default function LeaderboardSidebar({
  ...
  return (
    <aside className="flex w-65 shrink-0 flex-col gap-3 max-[860px]:w-full">
      <SidebarCard>
        <SidebarCardHeader
          icon={<SparklesIcon size={14} className="text-(--brand-500) dark:text-(--brand-500)" />}
          title="Scoring"
        />
        {scoringRules.map((rule) => (
          <div
            key={rule.source}
            className="flex items-center justify-between gap-4 border-b border-[#ece3db] py-2 last:border-b-0 dark:border-white/[0.07]"
          >
            <span className="text-[12px] text-(--text-secondary) dark:text-(--text-secondary)">
              {rule.label}
            </span>
            <span className="shrink-0 font-mono text-[11px] font-bold text-(--brand-500) tabular-nums dark:text-(--brand-500)">
              {rule.xpLabel}
            </span>
          </div>
        ))}
      ...
      <SidebarCard>
        <SidebarCardHeader
          icon={<TrophyIcon size={14} className="text-[#c49a2c]" />}
        ...
        {streakChampions.length > 0 ? (
          streakChampions.map((entry) => (
            <div
              key={entry.userId}
              className="flex items-center gap-2.5 border-b border-[#ece3db] py-2.25 last:border-b-0 dark:border-white/[0.07]"
            >
              <LeaderboardAvatar
                ...
                size="sm"
              />
              <span className="flex-1 truncate text-[12.5px] text-(--text-primary) dark:text-(--text-primary)">
                {entry.name}
              </span>
              <span className="flex items-center gap-1 font-mono text-[11px] text-[#b0a097] tabular-nums dark:text-[#6b6460]">
                <FireIcon size={11} /> {entry.streak}
              </span>
            </div>
          ))
        ) : (
          <p className="text-[12px] leading-relaxed text-[#9b8a82] dark:text-[#6b6460]">
            No streak champions are available yet.
          </p>
        )}
      </SidebarCard>
      ...
      <div className="rounded-2xl bg-(--brand-500) p-5 dark:bg-[#c65f43]">
        ...
        <p className="mb-4 text-[11.5px] leading-[1.6] text-[rgba(255,255,255,0.8)]">
          {reward.description}
        </p>
        <button
          type="button"
