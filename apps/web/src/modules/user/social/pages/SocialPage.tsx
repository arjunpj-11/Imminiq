...
export default function SocialPage() {
  ...
  const requestedSection = (searchParams.get('view') as ChatSection | null) ?? 'chats';
  const section = requestedSection === 'calls' && !features.calls ? 'chats' : requestedSection;
  ...
  const [friendToRemove, setFriendToRemove] = useState<IFriendUser | null>(null);
  ...
  const deferredMessageSearch = useDeferredValue(messageSearch.trim());
  const messageSearchQuery = useSearchChatMessages(selectedId, deferredMessageSearch);
  const callHistoryQuery = useCallHistory(
    CALL_HISTORY_PAGE_SIZE,
    features.calls && section === 'calls'
  );
  ...
  const retryMessage = useSendChatMessage(selectedId);
  const forwardMessage = useForwardChatMessage();
  const toggleMessageStar = useToggleChatMessageStar();
  const toggleMessageReaction = useToggleChatMessageReaction();
  ...
  const selectedUserBlockedViewer = Boolean(
    selectedConversation && blockedByUserIds.includes(selectedConversation.participant.id)
  );
  const conversationBlocked = viewerBlockedSelectedUser || selectedUserBlockedViewer;
  ...
  const visibleMessages = useMemo(
    () => (deferredMessageSearch ? (messageSearchQuery.data?.items ?? []) : messages),
    [deferredMessageSearch, messageSearchQuery.data?.items, messages]
  );
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      // Capture the boundary before the read mutation refreshes conversation counts.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setInitialUnreadCount(selectedConversation?.unreadCount ?? 0);
  ...
  }, [deferredMessageSearch, messagesQuery]);

  useEffect(() => {
    const root = messageCanvasRef.current;
  ...
  }, [loadEarlierMessages, selectedId]);

  useEffect(() => {
    const canvas = messageCanvasRef.current;
    if (!canvas || prependHeightRef.current === null || messagesQuery.isFetchingNextPage) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const refreshBlockState = (event?: {
      ...
      if (event?.blockerUserId && event.blockedUserId && typeof event.isBlocked === 'boolean') {
        queryClient.setQueryData<IUserBlocks>(chatQueryKeys.blocks(), (current) => {
          ...
          if (event.blockerUserId === viewerId) {
            return {
              ...current,
              blockedUserIds: updateIds(current.blockedUserIds, event.blockedUserId!),
            };
          }
          if (event.blockedUserId === viewerId) {
            return {
              ...current,
              blockedByUserIds: updateIds(current.blockedByUserIds, event.blockerUserId!),
            };
          }
          return current;
        });
  ...
  useEffect(() => {
    if (!showJumpButton && !isPrependingRef.current) {
  ...
  const tabs = [
    ...
    ...(features.calls ? ([['calls', 'Calls', PhoneCall]] as const) : []),
  ] as const;
  const activeTabLabel = tabs.find(([value]) => value === section)?.[1] ?? 'Chats';
  ...
  return (
    <AppShellBoundary withFooter={!selectedId}>
            ...
            {selectedConversation ? (
              <>
                <header className="relative z-10 flex h-[80px] shrink-0 items-center gap-3 border-b border-(--border-subtle) bg-(--surface-card)/95 px-3 shadow-[0_8px_28px_rgba(26,23,20,0.04)] backdrop-blur sm:px-5">
                  ...
                  {!conversationBlocked && features.calls && (
                    <>
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-(--border-subtle) bg-(--surface-elevated) px-3 text-[12px] font-bold text-(--text-secondary) transition hover:border-[rgba(184,76,43,0.28)] hover:text-(--brand-500)"
                      ...
                      {optionsOpen && (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[12px] font-semibold text-(--danger) hover:bg-(--surface-muted)"
                          >
                            <Trash2 size={14} /> Clear from my view
                          </button>
                          ...
                          <button
                            ...
                            className={cn(
                              'flex min-h-11 w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[12px] font-semibold hover:bg-(--surface-muted)',
                              viewerBlockedSelectedUser ? 'text-(--success)' : 'text-(--danger)'
                            )}
                          >
                            {viewerBlockedSelectedUser ? (
                              <UserCheck size={14} />
                ...
                >
                  ...
                  {messagesQuery.hasNextPage && !deferredMessageSearch && (
                    <div className="mb-4 flex justify-center">
                      <button
                        type="button"
                        onClick={loadEarlierMessages}
                        className="min-h-10 rounded-full border border-(--border-subtle) bg-(--surface-elevated) px-4 py-2 text-[11px] font-bold shadow-(--shadow-1)"
                      >
                    ...
                    >
                      {Array.from({ length: 6 }, (_, index) => (
                        ...
                        >
                          <div
                            className={cn(
                              'h-16 animate-pulse rounded-[20px] bg-(--surface-muted)',
                              index % 2 === 0 ? 'w-[58%]' : 'w-[42%]'
                            )}
                          />
                        </div>
                        ...
                        return (
                          <Fragment key={message.id}>
                            {!deferredMessageSearch &&
                              ...
                              index ===
                                Math.max(0, visibleMessages.length - initialUnreadCount) && (
                                <div className="my-2 flex items-center gap-3 text-[9px] font-bold text-(--brand-500)">
                                  <span className="h-px flex-1 bg-(--brand-500)/35" />
                                  Unread messages
                                  <span className="h-px flex-1 bg-(--brand-500)/35" />
                                </div>
                              )}
                            ...
                            <SocialMessageBubble
                              message={message}
                              mine={message.senderId === viewerId || Boolean(message.deliveryState)}
                              onRetry={(failed) => {
                                if (!failed.retryPayload) return;
                                if (failed.attachment?.url.startsWith('blob:')) {
                                  URL.revokeObjectURL(failed.attachment.url);
                                }
                                queryClient.setQueryData<InfiniteData<IChatPage<IChatMessage>>>(
                                  chatQueryKeys.messages(failed.conversationId),
                                  (current) =>
                                    current
                                      ? {
                                          ...current,
                                ...
                                retryMessage.mutate(failed.retryPayload);
                              }}
                              ...
                              onToggleStar={(target) => {
                                toggleMessageStar.mutate(target.id, {
                                  onSuccess: (updated) =>
                                    toast.success(
                                      updated.isStarred ? 'Message starred' : 'Star removed'
                                    ),
                                  onError: (error) =>
                                    toast.error(
                              ...
                              mutationPending={
                                toggleMessageReaction.isPending ||
                    ...
                    <div className="flex h-full items-center justify-center px-6 text-center">
                      <div className="rounded-3xl border border-(--border-subtle) bg-(--surface-elevated)/80 px-8 py-9 shadow-(--shadow-1) backdrop-blur">
                        <UserAvatar
                          ...
                          src={selectedConversation.participant.avatarUrl}
                          initials={selectedConversation.participant.initials}
                          profileUsername={selectedConversation.participant.username}
                  ...
                  {showJumpButton && (
                    <button
                      ...
                      className="sticky bottom-3 left-full inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-(--border-subtle) bg-(--surface-elevated) px-4 text-[10px] font-bold text-(--brand-500) shadow-(--shadow-2)"
                      aria-label="Jump to latest message"
                    >
                      <ArrowDown size={16} />
                      {initialUnreadCount > 0 ? `${initialUnreadCount} new` : 'Latest'}
                    </button>
                  )}
                </div>
                {conversationBlocked ? (
        ...
        >
          <header className="flex items-center gap-3 border-b border-(--border-subtle) px-5 py-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="mb-0 mt-1 text-[9px] text-(--text-muted)">Choose a conversation</p>
            </div>
            <button
              type="button"
      ...
      <ConfirmDialog
        ...
        description="This removes the message for everyone in the conversation. This action cannot be undone."
          ...
          deleteMessage.mutate(
            {
              ...
              conversationId: deletingMessage.conversationId,
            },
            {
              onSuccess: () => {
      ...
      <ConfirmDialog
        ...
        title="Clear unstarred messages from your view?"
        ...
        confirmText="Clear from my view"
        variant="danger"
        icon={<Trash2 size={20} />}
        ...
        onConfirm={() => {
          ...
          clearConversation.mutate(selectedConversation.id, {
            onSuccess: (result) => {
              toast.success(
                'Messages cleared from your view',
                result.clearedCount
      ...
      <ConfirmDialog
        ...
        description="They will be blocked and removed from your friends list. Unblocking will not restore the friendship; either person must send a new friend invite."
        confirmText="Block"
        variant="danger"
        icon={<UserX size={20} />}
      ...
      <RemoveFriendDialog
        ...
        onCancel={() => {
          if (!removeFriend.isPending) setFriendToRemove(null);
        }}
        onConfirm={() => {
          if (!friendToRemove) return;
          removeFriend.mutate(
            { friendUserId: friendToRemove.id },
            {
              onSuccess: () => {
                if (selectedConversation?.participant.id === friendToRemove.id) {
                  const next = new URLSearchParams(searchParams);
                  next.delete('conversation');
                ...
                toast.success('Friend removed', 'Send a new invite if you want to reconnect.');
