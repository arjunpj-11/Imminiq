    ...
    <PageContainer>
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="surface-flat relative min-w-0 overflow-hidden p-3.5 sm:p-4.5">
      ...
      <div className="grid grid-cols-[1fr_300px] gap-4 max-[900px]:grid-cols-1">
        <div className="rounded-xl border-[1.5px] border-(--border-subtle) bg-(--surface-card) p-6 shadow-(--shadow-1) max-[640px]:p-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <SkeletonBlock className="h-3 w-28" />
              <SkeletonBlock className="mt-2 h-6 w-2/3 rounded-lg" />
          ...
          <div className="space-y-2">
            {Array.from({ length: 4 }, (_, index) => (
              <div key={index} className="flex items-center gap-2.5 px-2.5 py-2">
                <SkeletonBlock className="h-8.5 w-8.5 shrink-0 rounded-full" />
      ...
      <section className="rounded-xl border-[1.5px] border-(--border-subtle) bg-(--surface-card) p-6 shadow-(--shadow-1) max-[640px]:p-4.5">
        ...
        <div className="overflow-hidden">
          <div className="flex min-w-180 gap-1">
            {Array.from({ length: 40 }, (_, week) => (
              <div key={week} className="grid grid-rows-7 gap-1">
                {Array.from({ length: 7 }, (_, day) => (
                  <SkeletonBlock key={day} className="h-3 w-3 rounded-xs" />
                ))}
              </div>
            ))}
          </div>
