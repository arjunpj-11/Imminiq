...
function getRenderedImageSize(imageSize: ISize, previewSize: ISize, scale: number): ISize {
  if (
    imageSize.width <= 0 ||
    imageSize.height <= 0 ||
    previewSize.width <= 0 ||
...
function clampOffset(
  ...
  const renderedImageSize = getRenderedImageSize(imageSize, previewSize, scale);

  if (renderedImageSize.width <= 0 || renderedImageSize.height <= 0) {
    return requestedOffset;
  }

  const maximumX = Math.max(0, (renderedImageSize.width - previewSize.width) / 2);

  const maximumY = Math.max(0, (renderedImageSize.height - previewSize.height) / 2);
...
export function useImageCropControls({
  ...
  const [previewSize, setPreviewSize] = useState<ISize>(EMPTY_SIZE);

  const [scaleState, setScaleState] = useState(() => clamp(initialScale, minScale, maxScale));

  const [rawOffset, setRawOffset] = useState<IOffset>(EMPTY_OFFSET);
  ...
  const offset = useMemo(
    () => clampOffset(rawOffset, imageSize, previewSize, scaleState),
    [imageSize, previewSize, rawOffset, scaleState]
  );
  ...
  const setImageSource = useCallback(
    ...
    [initialScale, maxScale, measurePreview, minScale]
  );
  ...
  const setScale = useCallback(
    (nextScale: number) => {
      ...
      setRawOffset((currentOffset) =>
        clampOffset(currentOffset, imageSize, previewSize, normalizedScale)
      );
    },
    [imageSize, maxScale, minScale, previewSize]
  );
  ...
  const handlePointerMove = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      ...
      setRawOffset(clampOffset(requestedOffset, imageSize, previewSize, scaleState));
    },
    [dragging, imageSize, previewSize, scaleState]
  );
  ...
  const handleWheel = useCallback(
    (event: WheelEvent<HTMLDivElement>) => {
      ...
      const direction = event.deltaY < 0 ? wheelStep : -wheelStep;

      const nextScale = clamp(Number((scaleState + direction).toFixed(2)), minScale, maxScale);

      setScaleState(nextScale);

      setRawOffset((currentOffset) =>
        clampOffset(currentOffset, imageSize, previewSize, nextScale)
      );
    },
    [imageSize, imageSrc, maxScale, minScale, previewSize, scaleState, wheelStep]
  );

  const renderedImageSize = useMemo(
    () => getRenderedImageSize(imageSize, previewSize, scaleState),
  ...
  const previewImageStyle = useMemo<CSSProperties | undefined>(() => {
    ...
    return {
      ...
      maxWidth: 'none',
      transform: ['translate(-50%, -50%)', `translate(${offset.x}px, ${offset.y}px)`].join(' '),
      transformOrigin: 'center center',
      userSelect: 'none',
      pointerEvents: 'none',
    };
  }, [offset.x, offset.y, renderedImageSize.height, renderedImageSize.width]);
  ...
  const renderToDataUrl = useCallback(
    async ({
      ...
      const outputRenderedSize = getRenderedImageSize(sourceSize, outputSize, scaleState);

      const outputOffset: IOffset = {
        x: offset.x * (width / measuredPreviewSize.width),
        y: offset.y * (height / measuredPreviewSize.height),
      };

      const drawX = (width - outputRenderedSize.width) / 2 + outputOffset.x;

      const drawY = (height - outputRenderedSize.height) / 2 + outputOffset.y;
      ...
      context.drawImage(image, drawX, drawY, outputRenderedSize.width, outputRenderedSize.height);

      return canvas.toDataURL(mimeType, quality);
    },
    [imageSrc, offset.x, offset.y, scaleState]
  );
