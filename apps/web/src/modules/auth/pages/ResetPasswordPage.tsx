    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-(--surface-canvas) px-4 py-12 font-ui text-(--text-primary) dark:bg-(--surface-canvas) dark:text-(--text-primary)">
        <div className="w-full max-w-110 rounded-xl border border-(--border-subtle) bg-(--surface-card) px-6 py-8 text-center shadow-(--shadow-2) dark:border-white/15 dark:bg-(--surface-card)">
          <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-[rgba(76,175,125,0.10)] text-(--success)">✓</div>
          <h1 className="font-serif text-[26px] font-bold">Password reset successful</h1>
          <p className="mt-2 text-[13px] leading-[1.6] text-(--text-secondary) dark:text-(--text-secondary)">You can now sign in with your new password.</p>
          <button
            type="button"
  ...
  return (
    <AuthLayout badge="Password Recovery" title="Create new password" subtitle="Choose a strong password for your Imminiq account.">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        <label className="block">
          ...
          <div className="relative">
            ...
            />
            <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-(--text-secondary) dark:text-(--text-secondary)" onClick={() => setShowNew((value) => !value)}>
        ...
        <label className="block">
          ...
          <div className="relative">
            <input
              ...
              autoComplete="new-password"
            />
            <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-(--text-secondary) dark:text-(--text-secondary)" onClick={() => setShowConfirm((value) => !value)}>
      ...
      <p className="mt-6 text-center text-[13px] text-(--text-secondary) dark:text-(--text-secondary)">
        Need a new code? <Link to="/forgot-password" className="text-(--brand-500) dark:text-(--brand-500)">Request again</Link>
      </p>
    </AuthLayout>
  )
}
