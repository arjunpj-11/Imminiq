  ...
  return (
    <AuthLayout badge="Create account" title="Join Imminiq" subtitle="Build your personalized learning roadmap.">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <label className="block">
          ...
          <div className="relative">
            ...
            />
            <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-(--text-secondary) transition hover:text-(--brand-500) dark:text-(--text-secondary) dark:hover:text-(--brand-500)" onClick={() => setShowPw((value) => !value)}>
              <EyeIcon open={showPw} />
            </button>
          </div>
          ...
          {form.password && (
            <div className="mt-2 flex items-center justify-between gap-3">
              <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-(--border-subtle) dark:bg-white/10">
                <div className="h-full rounded-full bg-(--brand-500) dark:bg-(--brand-500)" style={{ width: `${strength.level * 25}%` }} />
              </div>
        ...
        <label className="block">
          ...
          <div className="relative">
            <input
              ...
              autoComplete="new-password"
            />
            <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-(--text-secondary) transition hover:text-(--brand-500) dark:text-(--text-secondary) dark:hover:text-(--brand-500)" onClick={() => setShowCpw((value) => !value)}>
        ...
        <label className="flex items-start gap-2 text-[12.5px] leading-normal text-(--text-secondary) dark:text-(--text-secondary)">
          <input type="checkbox" name="terms" checked={form.terms} onChange={handleChange} onBlur={handleBlur} className="mt-0.5 accent-(--brand-500) dark:accent-(--brand-500)" />
          <span>
            I agree to the <Link to="/terms" className="text-(--brand-500) dark:text-(--brand-500)">Terms</Link> and{' '}
            <Link to="/privacy" className="text-(--brand-500) dark:text-(--brand-500)">Privacy Policy</Link>.
          </span>
        </label>
      ...
      <div className="my-5 flex items-center gap-3">
        ...
        <span className="font-mono text-[9px] uppercase tracking-[0.12em] text-(--text-secondary) opacity-60 dark:text-(--text-secondary)">Or</span>
        <div className="h-px flex-1 bg-(--border-subtle) dark:bg-white/15" />
      </div>
      <AuthSocialButtons />
