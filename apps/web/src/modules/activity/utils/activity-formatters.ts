...
export const parseActivityFilter = (
  value: string | null,
): ActivityFeedFilter =>
  value && VALID_FILTERS.has(value as ActivityFeedFilter)
    ? (value as ActivityFeedFilter)
    : 'all'
...
export const getBrowserUtcOffsetMinutes = (): number =>
  -new Date().getTimezoneOffset()

export const resolveAccountStartYear = (
  ...
  return Math.min(
    Math.max(accountDate.getFullYear(), ACTIVITY_MIN_YEAR),
    currentYear,
  )
}
