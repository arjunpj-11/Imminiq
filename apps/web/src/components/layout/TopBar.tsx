  ...
  const milestoneBase =
    streakDays < 7
      ...
      : streakDays < 14
        ...
        : streakDays < 30
          ? 14
          : Math.floor(streakDays / 10) * 10;
  const milestoneProgress = Math.max(
    0,
    Math.min(100, ((streakDays - milestoneBase) / Math.max(1, nextMilestone - milestoneBase)) * 100)
  );
            ...
            <>
              ...
              <div className="relative" ref={profileRef}>
                ...
                {profileOpen && (
                  <div
                    role="menu"
                    className="route-enter absolute right-0 top-[calc(100%+10px)] z-50 w-60 overflow-hidden rounded-lg border border-(--border-subtle) bg-(--surface-elevated) shadow-(--shadow-2)"
                  >
                    <div className="border-b border-(--border-subtle) p-3.5">
                      <div className="truncate text-[13px] font-[680] text-(--text-primary)">
                        {userName}
                      </div>
                      <div className="mt-0.5 truncate font-mono text-[9px] uppercase tracking-[0.08em] text-(--text-muted)">
                        {userLevel}
                      </div>
                    </div>
                    <div className="p-1.5">
