...
const LogoIcon = () => (
  <svg width="34" height="34" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" className="shrink-0 rounded-[10px]">
    ...
    <g transform="translate(-5,1)">
      ...
      <path d="M64 32.8C73.8 34.7 79.5 42.2 79.5 51.5 79.5 61.8 71.2 68 60.2 68c-7 0-12-2.5-15.1-7.2" fill="none" stroke="#fff8ed" strokeWidth="9" strokeLinecap="round" />
...
export default function Sidebar({ mobileOpen, collapsed, onCloseMobile, onToggleCollapsed }: SidebarProps) {
  return (
    <>
      <div
        className={cn(
      ...
      <aside
        className={cn(
          'fixed bottom-0 left-0 top-0 z-[30] flex w-[224px] flex-col border-r border-[#e0d0c5] bg-[#fdf8f5] shadow-[0_16px_56px_rgba(0,0,0,0.15)] transition-transform duration-300 ease-in-out dark:border-white/[0.09] dark:bg-[#1a1816] min-[901px]:shadow-none',
          mobileOpen ? 'max-[900px]:translate-x-0' : 'max-[900px]:-translate-x-full',
          collapsed ? 'min-[901px]:-translate-x-[224px]' : 'min-[901px]:translate-x-0'
        )}
      >
        <div className="flex items-center gap-2.5 border-b border-[#e0d0c5] px-5 pb-3.5 pt-[18px] dark:border-white/[0.09]">
          <LogoIcon />
        ...
        <nav className="flex-1 overflow-y-auto px-2.5 py-3.5">
          ...
          {navItems.map((item) => (
            <a
              key={item.label}
              href="#"
              className="pointer-events-none mb-px flex cursor-default items-center justify-between rounded-[9px] px-2.5 py-[9px] text-[13px] font-medium text-[#6b5f58] opacity-60 no-underline dark:text-[#9b9a92]"
              tabIndex={-1}
              aria-disabled="true"
            >
              ...
              {'kbd' in item && item.kbd ? (
                <span className="rounded bg-[rgba(26,23,20,0.09)] px-[5px] py-0.5 font-['DM_Mono',monospace] text-[9px] text-[#6b5f58] opacity-60 dark:bg-[rgba(242,240,235,0.09)] dark:text-[#9b9a92]">
                  {item.kbd}
                </span>
              ) : null}
            </a>
        ...
        <div className="relative mx-2.5 mb-4 overflow-hidden rounded-[14px] border border-[rgba(184,76,43,0.14)] bg-[rgba(184,76,43,0.07)] p-3.5 dark:border-[rgba(232,129,106,0.16)] dark:bg-[rgba(232,129,106,0.07)]">
          ...
          <div className="relative z-[1] mb-[3px] text-[12px] font-bold text-[#b84c2b] dark:text-[#e8816a]">Upgrade to Pro</div>
          <p className="relative z-[1] mb-2.5 text-[11px] leading-[1.45] text-[#6b5f58] dark:text-[#9b9a92]">
            Unlock advanced insights, AI evaluations, and unlimited tracker sharing.
          </p>
          <button
            type="button"
            className="relative z-[1] w-full rounded-lg border-none bg-[#b84c2b] p-[9px] font-['DM_Sans',sans-serif] text-[12px] font-bold text-[#fdf8f5] transition hover:bg-[#963d22] dark:bg-[#e8816a] dark:text-[#141412] dark:hover:bg-[#d4705a]"
          >
            Upgrade
          </button>
        </div>
