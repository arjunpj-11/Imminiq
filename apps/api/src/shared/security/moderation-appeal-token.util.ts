...
export const createModerationAppealToken = (userId: string, identifier: string): string =>
  jwt.sign({ userId, identifier, purpose: 'moderation_appeal' }, env.JWT_SECRET, {
    algorithm: 'HS256',
...
export const verifyModerationAppealToken = (token: string): AppealTokenPayload => {
  try {
    ...
    throw new ApiError(
      401,
      'Appeal authorization is invalid or expired. Please sign in again.',
