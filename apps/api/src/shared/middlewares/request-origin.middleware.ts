 ...
 */
export const verifyBrowserRequestOrigin = (req: Request, _res: Response, next: NextFunction) => {
  ...
  if (requestOrigin && requestOrigin !== clientOrigin) {
    next(new ApiError(403, 'Request origin is not allowed', 'REQUEST_ORIGIN_REJECTED'));
    return;
  }

  if (!requestOrigin && requestRefererOrigin && requestRefererOrigin !== clientOrigin) {
    next(new ApiError(403, 'Request referer is not allowed', 'REQUEST_REFERER_REJECTED'));
    return;
  }
