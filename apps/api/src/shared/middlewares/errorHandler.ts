...
const isHttpOperationalError = (err: Error): err is HttpOperationalError => {
...
const buildErrorResponse = (err: HttpOperationalError) => {
  return {
    success: false,
...
export const errorHandler = (err: Error, _req: Request, res: Response, _next: NextFunction) => {
  ...
  if (err instanceof ApiError) {
    return res.status(err.statusCode).json({
      ...
      message: err.statusCode >= 500 ? 'Internal server error' : err.message,
      code: err.code,
      ...('errors' in err && err.errors ? { errors: err.errors as ErrorDetails } : {}),
    });
  }
