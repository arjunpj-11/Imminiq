...
export interface MockTestsRepositoryContract {
  ...
  ): Promise<{ tests: MockTest[]; total: number }>

  findPublicTests(filters: {
  ...
  findSharedTestByToken(shareToken: string): Promise<MockTest | null>
 ...
 createQuestions(
  ...
  findLatestAttemptsForTests(
    userId: string,
    testIds: string[],
  ): Promise<Record<string, MockTestAttempt>>
  ...
  getPerformanceTrends(
    userId: string,
  ): Promise<{ date: string; averageScore: number; attempts: number }[]>

  getTopicBreakdown(
    userId: string,
  ): Promise<{ topic: string; averageScore: number; totalAttempts: number }[]>
  ...
  updateCreationSession(
    sessionId: string,
    data: Partial<MockTestCreationSession>,
  ): Promise<MockTestCreationSession | null>
