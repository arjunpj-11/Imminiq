...
for (let i = 0; i < __imminiqTestCases.length; i += 1) {
  ...
  try {
    if (typeof __imminiqTarget !== 'function') {
      throw new Error('Function ${escapeForSingleQuotedString(coding.functionName)} was not found')
    }
...
export const runMockTestCodingQuestion = async ({
  sourceCode,
  coding,
  mode,
  language,
