...
export class SetupTwoFactorUseCase implements ISetupTwoFactorUseCase {
  ...
  async execute(userId: string): Promise<ITwoFactorSetupResponseDTO> {
    ...
    const existingTwoFactor = await this._securityRepository.findTwoFactorByUserId(userId);
    ...
    const setupRecord = await this._securityRepository.savePendingTwoFactorSetup({
      userId,
      data: {
        encryptedSecret,
        issuer: TWO_FACTOR_ISSUER,
