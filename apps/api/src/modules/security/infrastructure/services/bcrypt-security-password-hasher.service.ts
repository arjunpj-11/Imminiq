...
export class BcryptSecurityPasswordHasher implements ISecurityPasswordHasher {
  async hash(value: string): Promise<string> {
    try {
      ...
      throw new SecurityDomainError('PASSWORD_HASH_FAILED', 'Password hashing failed');
    }
  }
  ...
  async compare(value: string, hashedValue: string): Promise<boolean> {
    try {
      ...
      throw new SecurityDomainError('PASSWORD_COMPARE_FAILED', 'Password verification failed');
    }
  }
}

export const bcryptSecurityPasswordHasher = new BcryptSecurityPasswordHasher();
