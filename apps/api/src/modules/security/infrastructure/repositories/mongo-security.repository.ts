...
export class MongoSecurityRepository implements ISecurityRepository {
  ...
  constructor(
    ...
    this._userRepository = dependencies.userRepository ?? new MongoSecurityUserRepository(mapper);

    this._sessionRepository =
      dependencies.sessionRepository ?? new MongoSecuritySessionRepository(mapper);

    this._twoFactorRepository =
      dependencies.twoFactorRepository ?? new MongoSecurityTwoFactorRepository(mapper);
  }
  ...
  findCurrentSessionByRefreshTokenHash(refreshTokenHash: string) {
    return this._sessionRepository.findCurrentSessionByRefreshTokenHash(refreshTokenHash);
  }
