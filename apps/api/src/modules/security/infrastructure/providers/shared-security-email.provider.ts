...
export class SharedSecurityEmailProvider implements ISecurityEmailProvider {
  ...
  async sendEmailChangeAlert(to: string, input: EmailChangeAlertTemplateInput): Promise<void> {
    try {
      await sendMail(to, 'Imminiq email change requested', emailChangeAlertTemplate(input));
    } catch {
      throw new SecurityDomainError(
