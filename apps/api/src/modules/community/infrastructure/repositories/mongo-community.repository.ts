  ...
  implements CommunityRepositoryContract
{
  ...
  async submitTrackerForVerification(data: SubmitTrackerForVerificationInput) {
    return this.execute(
      ...
      async () => {
        ...
        if (
          !this.isValidObjectId(data.trackerId) ||
          !this.isValidObjectId(data.userId)
        ) {
          return null
        }

        const trackerObjectId = this.toObjectId(data.trackerId)
  ...
  private async getVerificationReviewTracker(
    ...
    const trackerId = this.toExistingObjectId(submission.trackerId)
