...
export interface IGetMyBadgesUseCase {
  execute(userId: string, page: number, limit: number): Promise<{ items: import("..").IEarnedBadgeViewDTO[]; pagination: { page: number; limit: number; total: number; totalPages: number; }; }>
}
