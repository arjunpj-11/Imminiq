...
export class RedisSecurityAttemptStore implements ISecurityAttemptStore {
  async isBlocked(scope: SecurityAttemptScope, identifier: string): Promise<boolean> {
    return securityAttemptCache.isBlocked(scope, identifier);
  }

  async getRetryAfterSeconds(scope: SecurityAttemptScope, identifier: string): Promise<number> {
