...
import { UploadsMapper } from './application/uploads.mapper';
import { AIUploadPromptBuilder } from './application/services/ai-upload-prompt.service';
import { UploadUserProfileReader } from './application/services/upload-user-profile.service';
...
export const createUploadsComposition = (usersProfileReader: IGetMeUseCase): UploadsComposition => {
  ...
  return {
    useCases: {
      ...
      removeAvatar: new RemoveAvatarUseCase(userProfileReader, uploadsRepository),

      removeBanner: new RemoveBannerUseCase(userProfileReader, uploadsRepository),
