...
export class MongoUploadsErrorMapper {
  static mapDuplicateUploadRecordError(error: unknown): UploadsDomainError | null {
    if (!MongoUploadsErrorMapper.isDuplicateKeyError(error)) {
      return null;
    }

    return new UploadsDomainError('DUPLICATE_UPLOAD_RECORD', 'Duplicate upload record');
  }
