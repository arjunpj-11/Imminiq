...
const ALLOWED_TYPES_BY_CATEGORY: Record<
  ActivityCategory,
...
export class ActivityEventPolicy {
  ensureValid(payload: RecordUserActivityPayloadDTO): void {
    ...
    if (subtitle.length > 300) {
      throw ActivityApplicationError.invalidEvent(
        'Activity subtitle cannot exceed 300 characters',
      )
    }
    ...
    if (
      !ALLOWED_TYPES_BY_CATEGORY[payload.category].includes(
        payload.type,
      )
    ) {
      throw ActivityApplicationError.invalidEvent(
    ...
    if (!Number.isInteger(xpAwarded) || xpAwarded < 0) {
      throw ActivityApplicationError.invalidEvent(
        'Activity XP must be a non-negative integer',
      )
    }

    if (!Number.isInteger(coinsAwarded) || coinsAwarded < 0) {
      throw ActivityApplicationError.invalidEvent(
        'Activity coins must be a non-negative integer',
      )
    }
    ...
    if (xpAwarded === 0 && xpBucket !== 'none') {
      throw ActivityApplicationError.invalidEvent(
        'XP bucket must be none when no XP is awarded',
      )
    }
  ...
  private ensureValidDetails(
    ...
    if (
      ...
      (details.scorePercentage < 0 ||
        details.scorePercentage > 100)
    ) {
      throw ActivityApplicationError.invalidEvent(
