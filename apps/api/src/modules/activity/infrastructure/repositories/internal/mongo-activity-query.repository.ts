  ...
  implements ActivityQueryRepositoryContract
{
  ...
  async findActivityFeed(
    ...
    return this.execute(
      ...
      async () => {
        ...
        return {
          activities: pageRecords.map(
            (record: MongoUserActivityRecord) =>
    ...
    return this.execute(
      ...
      async () => {
        ...
        const [
          ...
          this.findLeaderboardDayXp(
            userId,
            input.currentWeekRange,
            input.timezone,
          ),

          this.findLeaderboardRangeXp(
            userId,
          ...
          this.findLeaderboardBreakdown(
            userId,
            input.currentWeekRange,
          ),

          this.findActivityBreakdown(
            userId,
            input.currentWeekRange,
          ),

          this.findLeaderboardBestDayXp(
            userId,
            input.timezone,
          ),

          this.findActivityBestDayXp(
            userId,
            input.timezone,
          ),
        ...
        const previousWeekXp = Math.max(
          previousWeekLeaderboardXp.xp,
          previousWeekActivityXp,
        )
        ...
        const currentWeekBreakdown =
          this.mergeBreakdowns(
            currentWeekActivityBreakdown,
            currentWeekLeaderboardBreakdown.value,
          )

        const bestDayXp = Math.max(
          leaderboardBestDay?.xp ?? 0,
          activityBestDay?.xp ?? 0,
        )

        const userStreakCount = Math.max(
          0,
          user?.streakCount ?? 0,
        )
  ...
  private async findDayAggregates(
    ...
    return records.map(
      (record: MongoActivityDayAggregateRecord) => ({
        date: record._id,
        activityCount:
  ...
  private async findLeaderboardDayXp(
    ...
    return LeaderboardXpEvent.aggregate<
      LeaderboardDayXpRecord
    >([
      {
        $match: {
          userId,
          occurredAt:
            MongoActivityFilterBuilder.dateRange(range),
        },
      },
      {
        $group: {
  ...
  private async findLeaderboardRangeXp(
    ...
    const [record] =
      await LeaderboardXpEvent.aggregate<
        LeaderboardRangeXpRecord
      >([
        {
          $match: {
            userId,
  ...
  private async findStreakAnalytics(
    ...
    const hasStoredStreak =
      Boolean(latestHistory || latestSnapshot)
     ...
     */
    const legacyActiveDateKeys =
      await this.findAllActiveDateKeys(
        userId,
        input.timezone,
      )
    ...
    const legacyLatestActivityDate =
      legacyActiveDateKeys.length > 0
        ? legacyActiveDateKeys[
            legacyActiveDateKeys.length - 1
          ] ?? null
        : null

    const activityDates = [
      storedLatestActivityDate,
      legacyLatestActivityDate,
    ]
    ...
    const currentIsActive =
      latestActivityDate === input.todayKey ||
      latestActivityDate === input.yesterdayKey

    const storedCurrent = Math.max(
      0,
      latestHistory?.streakDay ??
        latestSnapshot?.currentStreak ??
        0,
    )
    ...
    const legacyYearDays =
      await this.findLegacyStreakDays(
        userId,
        input.yearRange,
        input.timezone,
      )
    ...
    const daysByDate = new Map(
      legacyYearDays.map((day) => [day.date, day]),
    )
    ...
    for (const historyDay of historyDays) {
      ...
      const activityCount = Math.max(
        legacyDay?.activityCount ?? 0,
        historyDay.activityCount ?? 0,
      )
      ...
      daysByDate.set(date, {
        ...
        activityCount,
        intensityLevel:
          this.heatmapIntensity(activityCount),
        isFrozen: historyDay.isFrozen ?? false,
        streakDay: Math.max(
          0,
          historyDay.streakDay ??
            legacyDay?.streakDay ??
            0,
        ),
      })
    ...
    const days = [...daysByDate.values()].sort(
    ...
    return {
      currentStreak: currentIsActive
        ? Math.max(
            hasStoredStreak ? storedCurrent : 0,
            legacyStreak.currentStreak,
          )
        : 0,

      longestStreak: Math.max(
        storedLongest,
        legacyStreak.longestStreak,
      ),
  ...
  private async findLegacyStreakDays(
    ...
    const days = await this.findDayAggregates(
      userId,
      range,
      timezone,
    )
    ...
    return days.map((day) => ({
      ...
      intensityLevel:
        this.heatmapIntensity(day.activityCount),
      isFrozen: false,
      streakDay: 0,
    }))
  }
  ...
  private calculateStreak(
    ...
    for (
      ...
      const difference =
        this.dateKeyToTimestamp(current) -
        this.dateKeyToTimestamp(previous)
