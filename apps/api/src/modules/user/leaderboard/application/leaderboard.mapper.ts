...
import type { LeaderboardEntryViewDTO, LeaderboardTopThreeViewDTO } from './leaderboard.dto';
...
export interface ILeaderboardMapper {
  toEntryView(entry: LeaderboardEntryEntity, viewerUserId: string): LeaderboardEntryViewDTO;

  toTopThreeView(entry: LeaderboardEntryEntity, viewerUserId: string): LeaderboardTopThreeViewDTO;
}
...
export class LeaderboardMapper implements ILeaderboardMapper {
  toEntryView(entry: LeaderboardEntryEntity, viewerUserId: string): LeaderboardEntryViewDTO {
    return {
      ...
      track: entry.section === 'students' ? `Level ${entry.level}` : `Trainer Lv. ${entry.level}`,
      xp: entry.score,
