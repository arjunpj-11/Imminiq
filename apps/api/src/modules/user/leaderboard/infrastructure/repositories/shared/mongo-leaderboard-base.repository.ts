...
export abstract class MongoLeaderboardBaseRepository {
  protected async execute<T>(
    ...
    try {
      ...
      throw new LeaderboardDomainError(code, message);
    }
  }

  private logRepositoryError(code: string, message: string, error: unknown): void {
    if (!(error instanceof Error)) {
      console.error('Leaderboard repository operation failed', {
        code,
        message,
        originalError: error,
      });
