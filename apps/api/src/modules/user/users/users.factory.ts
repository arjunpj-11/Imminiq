...
import { UsersMapper } from './application/users.mapper';
import { UsersProfileDataReader } from './application/services/users-profile-data.service';
...
export const createUsersComposition = (): UsersComposition => {
  ...
  const profileDataReader = new UsersProfileDataReader(usersRepository, usersMapper, systemClock);

  return {
    useCases: {
      getMe: new GetMeUseCase(usersRepository, usersMapper),

      updateMe: new UpdateMeUseCase(usersRepository, usersMapper),

      getUserByUsername: new GetUserByUsernameUseCase(usersRepository, usersMapper),

      getMyStats: new GetMyStatsUseCase(profileDataReader),

      getMyRecentActivity: new GetMyRecentActivityUseCase(usersRepository, usersMapper),

      getMyStreak: new GetMyStreakUseCase(profileDataReader),

      getMyPublishedTrackers: new GetMyPublishedTrackersUseCase(usersRepository, usersMapper),

      getMyBadges: new GetMyBadgesUseCase(usersRepository, usersMapper),
