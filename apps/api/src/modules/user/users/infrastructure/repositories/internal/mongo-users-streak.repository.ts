...
export class MongoUsersStreakRepository extends MongoUsersBaseRepository {
  ...
  async findLatestSnapshot(userId: UserIdInput): Promise<UserStreakSnapshotEntity | null> {
    return this.execute(
  ...
  async findHistoryByYear(input: FindUserStreakHistoryByYearInput): Promise<UserStreakDayEntity[]> {
    return this.execute(
      'USER_STREAK_READ_FAILED',
