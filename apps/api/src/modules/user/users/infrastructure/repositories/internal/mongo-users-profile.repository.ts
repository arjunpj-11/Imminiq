...
import type { MongoPrivacySettingsRecord, MongoProfileRecord } from '../shared/mongo-users.types';
...
export class MongoUsersProfileRepository extends MongoUsersBaseRepository {
  ...
  async findByUserId(userId: UserIdInput): Promise<UserProfileEntity | null> {
    return this.execute('USER_PROFILE_READ_FAILED', 'Failed to read user profile', async () => {
      const profile = await UserProfile.findOne({
        userId: MongoUsersObjectId.from(userId),
        ...MONGO_USERS_ACTIVE_FILTER,
      }).lean<MongoProfileRecord>();

      return profile ? this._mapper.toUserProfileEntity(profile, userId) : null;
    });
  }

  async findPrivacySettings(userId: UserIdInput): Promise<UserPrivacySettingsEntity | null> {
    return this.execute(
  ...
  async ensureForUser(input: EnsureUserProfileInput): Promise<UserProfileEntity> {
    return this.execute(
  ...
  async updateByUserId(input: UpdateUserProfileInput): Promise<UserProfileEntity | null> {
    return this.execute(
      ...
      async () => {
        ...
        return profile ? this._mapper.toUserProfileEntity(profile, input.userId) : null;
      },
      MongoUsersErrorMapper.mapDuplicateUserRecordError
    );
  }
