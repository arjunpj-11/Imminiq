...
export class MongoUsersRepository implements IUsersRepository {
  ...
  constructor(
    ...
    this._userRepository = dependencies.userRepository ?? new MongoUsersUserRepository(mapper);

    this._profileRepository =
      dependencies.profileRepository ?? new MongoUsersProfileRepository(mapper);

    this._activityRepository =
      dependencies.activityRepository ?? new MongoUsersActivityRepository(mapper);

    this._badgeRepository = dependencies.badgeRepository ?? new MongoUsersBadgeRepository(mapper);

    this._streakRepository =
      dependencies.streakRepository ?? new MongoUsersStreakRepository(mapper);

    this._trackerRepository =
      dependencies.trackerRepository ?? new MongoUsersTrackerRepository(mapper);

    this._relationshipRepository =
      dependencies.relationshipRepository ?? new MongoUsersRelationshipRepository();
  }
  ...
  updateFullName(input: UpdateUserFullNameInput): Promise<UserEntity | null> {
  ...
  findRecentActivity(input: FindRecentUserActivityInput): Promise<UserActivityEntity[]> {
  ...
  findLatestSnapshot(userId: UserIdInput): Promise<UserStreakSnapshotEntity | null> {
    return this._streakRepository.findLatestSnapshot(userId);
  }

  findHistoryByYear(input: FindUserStreakHistoryByYearInput): Promise<UserStreakDayEntity[]> {
