...
import type { MongoBadgeRecord, MongoEarnedBadgeRecord } from '../shared/mongo-users.types';
...
export class MongoUsersBadgeRepository extends MongoUsersBaseRepository {
  ...
  async findBadgeShowcase(userId: UserIdInput): Promise<{
    ...
    return this.execute(
      ...
      async () => {
        ...
        return {
          catalog: catalog.map((badge) => this._mapper.toUserBadgeEntity(badge)),
          earned: earned.map((item) => this._mapper.toEarnedUserBadgeEntity(item)),
        };
      }
    );
  }
  ...
  async findEarnedBadgesPaginated(
    ...
    return this.execute('USER_BADGE_READ_FAILED', 'Failed to read earned user badges', async () => {
      ...
      return {
        items: items.map((item) => this._mapper.toEarnedUserBadgeEntity(item)),
        total,
      };
    });
  }
