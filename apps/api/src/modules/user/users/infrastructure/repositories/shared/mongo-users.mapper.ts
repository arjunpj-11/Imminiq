...
export class MongoUsersMapper {
  ...
  toUserProfileEntity(profile: MongoProfileRecord, fallbackUserId: UserIdInput): UserProfileEntity {
  ...
  toUserActivityEntity(activity: MongoActivityRecord): UserActivityEntity {
    if (!activity.createdAt) {
      throw new UsersDomainError('MAPPING_FAILED', 'User activity is missing its creation date');
    }
  ...
  toUserBadgeEntity(badge: MongoBadgeRecord): UserBadgeEntity {
    if (!badge.badgeType) {
      throw new UsersDomainError('MAPPING_FAILED', 'User badge is missing its badge type');
    }
  ...
  toEarnedUserBadgeEntity(earnedBadge: MongoEarnedBadgeRecord): EarnedUserBadgeEntity {
    if (!this.isMongoBadgeRecord(earnedBadge.badgeId)) {
      throw new UsersDomainError('MAPPING_FAILED', 'Earned badge details were not populated');
    }
  ...
  toStreakSnapshotEntity(snapshot: MongoStreakSnapshotRecord): UserStreakSnapshotEntity {
  ...
  toStreakDayEntity(day: MongoStreakHistoryRecord): UserStreakDayEntity {
    if (!day.date) {
      throw new UsersDomainError('MAPPING_FAILED', 'Streak history is missing its date');
    }
  ...
  toPublishedTrackerEntity(tracker: MongoTrackerRecord): PublishedTrackerEntity {
  ...
  toProfileUpdateData(payload: UserProfileUpdate): Record<string, unknown> {
    return {
      ...
      ...(payload.headline !== undefined ? { headline: payload.headline.trim() } : {}),
      ...(payload.bio !== undefined ? { bio: payload.bio.trim() } : {}),
      ...(payload.location !== undefined ? { location: payload.location.trim() } : {}),
      ...(payload.education !== undefined ? { education: payload.education.trim() } : {}),
      ...(payload.skills !== undefined ? { skills: payload.skills } : {}),
      ...(payload.interests !== undefined ? { interests: payload.interests } : {}),
      ...(payload.githubUrl !== undefined ? { githubUrl: payload.githubUrl.trim() } : {}),
      ...(payload.linkedinUrl !== undefined ? { linkedinUrl: payload.linkedinUrl.trim() } : {}),
      ...(payload.portfolioUrl !== undefined ? { portfolioUrl: payload.portfolioUrl.trim() } : {}),
  ...
  buildTrackerSort(sortBy: PublishedTrackerQuery['sort']): Record<string, 1 | -1> {
