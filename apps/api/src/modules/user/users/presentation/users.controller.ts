...
export class UsersController {
  ...
  updateMe = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this._useCases.updateMe.execute(getAuthUser(req).userId, req.body);
  ...
  getUserByUsername = async (req: Request<UsernameParams>, res: Response, next: NextFunction) => {
    try {
  ...
  getPublicProfile = async (req: Request<UsernameParams>, res: Response, next: NextFunction) => {
    try {
  ...
  getMyRecentActivity = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this._useCases.getMyRecentActivity.execute(
        getAuthUser(req).userId,
        this.clampInteger(req.query.limit, USERS_DEFAULT_LIMIT, 1, USERS_MAX_LIMIT)
      );
    ...
    try {
      ...
      const year =
        typeof req.query.year === 'string'
          ? this.clampInteger(req.query.year, currentYear, USERS_MIN_STREAK_YEAR, currentYear + 1)
          : undefined;

      const result = await this._useCases.getMyStreak.execute(getAuthUser(req).userId, year);
  ...
  getMyPublishedTrackers = async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  private buildTrackerPaginationQuery(req: Request): PaginationQueryDTO {
    ...
    return {
      ...
      limit: this.clampInteger(req.query.limit, USERS_DEFAULT_LIMIT, 1, USERS_MAX_LIMIT),
      ...(search ? { search } : {}),
      ...(status ? { status } : {}),
