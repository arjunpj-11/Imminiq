...
import type { FriendActionViewDTO, RemoveFriendPayloadDTO } from '../friends.dto';
...
export class RemoveFriendUseCase implements IRemoveFriendUseCase {
  ...
  async execute(userId: string, payload: RemoveFriendPayloadDTO): Promise<FriendActionViewDTO> {
    this._friendRelationshipPolicy.ensureDifferentUsers(userId, payload.friendUserId);
