  ...
  implements IFriendQueryRepository
{
  ...
  async searchUsers(input: SearchFriendUsersInput): Promise<FriendUsersPage> {
    return this.execute('FRIEND_USER_SEARCH_FAILED', 'Failed to search users', async () => {
      ...
      const searchPattern = new RegExp(MongoFriendsNormalizer.escapeRegex(normalizedQuery), 'i');
  ...
  async listFriends(input: ListFriendsInput): Promise<FriendUsersPage> {
    return this.execute('FRIEND_LIST_READ_FAILED', 'Failed to read friends', async () => {
      const viewerUserId = MongoFriendsNormalizer.toObjectId(
        input.viewerUserId,
        'INVALID_VIEWER_USER_ID'
      );
      const skip = (input.page - 1) * input.limit;
      const normalizedSearch = input.search ? MongoFriendsNormalizer.search(input.search) : '';
      const searchPattern = normalizedSearch
        ? new RegExp(MongoFriendsNormalizer.escapeRegex(normalizedSearch), 'i')
        : null;
      ...
      if (searchPattern) {
        pipeline.push({
          $match: {
            $or: [{ 'user.fullName': searchPattern }, { 'user.username': searchPattern }],
          },
        });
      }
      ...
      const [facet] = await Friend.aggregate<MongoFriendListFacetRecord>(pipeline);

      const records = (facet?.items ?? []).map((item) => item.user);
      const mutualCounts = await this.findMutualCounts(
        viewerUserId,
        records.map((record) => MongoFriendsNormalizer.toObjectId(this._mapper.toId(record._id)))
      );
  ...
  async findFriendUser(viewerUserIdValue: string, friendUserIdValue: string) {
    return this.execute('FRIEND_USER_READ_FAILED', 'Failed to read friend user', async () => {
      const viewerUserId = MongoFriendsNormalizer.toObjectId(
        viewerUserIdValue,
        'INVALID_VIEWER_USER_ID'
      );
      ...
      const mutualCounts = await this.findMutualCounts(viewerUserId, [friendUserId]);
  ...
  private async findRelationshipStates(
    ...
    for (const request of pendingRequests) {
      ...
      const counterpartId = senderId === viewerUserId.toString() ? receiverId : senderId;
      ...
      states.set(
        counterpartId,
        senderId === viewerUserId.toString() ? 'pending_sent' : 'pending_received'
      );
    }
  ...
  private async findMutualCounts(
    ...
    const viewerFriendIds = viewerFriendRecords.map((record) => record.friendId);
