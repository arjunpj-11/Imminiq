...
export class MongoFriendsMapper {
  ...
  toFriendRequestEntity(record: MongoFriendRequestRecord | null): FriendRequestEntity | null {
    if (!record) {
      return null;
    }
  ...
  toFriendRequestEntityOrThrow(record: MongoFriendRequestRecord | null): FriendRequestEntity {
    const entity = this.toFriendRequestEntity(record);

    if (!entity) {
      throw new FriendsDomainError('FRIEND_REQUEST_MAPPING_FAILED', 'Failed to map friend request');
    }
  ...
  toFriendRequestSummaryEntity(
    ...
    const relationshipStatus = direction === 'received' ? 'pending_received' : 'pending_sent';
