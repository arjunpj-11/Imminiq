...
import type { FriendRequestsPage, ListFriendRequestsInput } from '../../../domain/friends.types';
  ...
  private async listRequests(
    ...
    return this.execute(
      ...
      async () => {
        ...
        const counterpartField = direction === 'received' ? '$senderId' : '$receiverId';
        const ownershipMatch =
          direction === 'received' ? { receiverId: viewerUserId } : { senderId: viewerUserId };
        ...
        const mutualCounts = await this.findMutualCounts(viewerUserId, candidateIds);
