    ...
    return this.execute(
      ...
      async () => {
        ...
        const battles = (await Battle.find({
          ...
          .limit(MongoDashboardQueryUtils.safeLimit(limit, DASHBOARD_DEFAULT_RECENT_BATTLES_LIMIT))
          .select(
        ...
        const opponentIds = battles.map((battle) => this._mapper.getOpponentId(battle, userId));
        ...
        const opponentMap = new Map(
          opponents.map((opponent) => [this._mapper.toId(opponent._id), opponent])
        );
        ...
        return battles.map((battle) =>
          this._mapper.toDashboardBattleEntity(battle, userId, opponentMap, profileMap)
        );
      },
...
export const mongoDashboardBattleRepository = new MongoDashboardBattleRepository();
