...
export class MongoDashboardNotificationRepository extends MongoDashboardBaseRepository {
  ...
  async getRecentActivity(input: GetRecentActivityInput): Promise<DashboardRecentActivityEntity[]> {
    return this.execute(
      ...
      async () => {
        const { userId, limit = DASHBOARD_DEFAULT_RECENT_ACTIVITY_LIMIT } = input;
        ...
        const notifications = await Notification.find({
          ...
          .sort({ createdAt: -1 })
          .limit(MongoDashboardQueryUtils.safeLimit(limit, DASHBOARD_DEFAULT_RECENT_ACTIVITY_LIMIT))
...
export const mongoDashboardNotificationRepository = new MongoDashboardNotificationRepository();
