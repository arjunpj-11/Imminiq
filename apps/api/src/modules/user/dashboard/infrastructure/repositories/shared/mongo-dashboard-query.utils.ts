...
export class MongoDashboardQueryUtils {
  ...
  static safeLimit(limit: number | undefined, fallback: number): number {
    ...
    return Math.max(1, Math.min(DASHBOARD_MAX_RECENT_ITEMS_LIMIT, Math.trunc(limit)));
  }

  static toObjectId(id: string): mongoose.Types.ObjectId {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw new DashboardDomainError('DASHBOARD_INVALID_USER_ID', 'Invalid dashboard user id');
    }
