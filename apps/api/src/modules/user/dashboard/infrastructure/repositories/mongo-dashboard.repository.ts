...
export class MongoDashboardRepository implements IDashboardRepository {
  ...
  constructor(
    ...
    this._userRepository = dependencies.userRepository ?? new MongoDashboardUserRepository(mapper);

    this._streakRepository =
      dependencies.streakRepository ?? new MongoDashboardStreakRepository(mapper);

    this._trackerRepository =
      dependencies.trackerRepository ?? new MongoDashboardTrackerRepository(mapper);

    this._notificationRepository =
      dependencies.notificationRepository ?? new MongoDashboardNotificationRepository(mapper);

    this._battleRepository =
      dependencies.battleRepository ?? new MongoDashboardBattleRepository(mapper);

    this._friendRepository =
      dependencies.friendRepository ?? new MongoDashboardFriendRepository(mapper);
  }
  ...
  async findProfileByUserId(userId: string): Promise<DashboardProfileEntity | null> {
  ...
  async getRecentActivity(input: GetRecentActivityInput): Promise<DashboardRecentActivityEntity[]> {
  ...
  async getRecentBattles(input: GetRecentBattlesInput): Promise<DashboardBattleEntity[]> {
    return this._battleRepository.getRecentBattles(input);
  }

  async getFriendsHub(input: GetFriendsHubInput): Promise<DashboardFriendEntity[]> {
