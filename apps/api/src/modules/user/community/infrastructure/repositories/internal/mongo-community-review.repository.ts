  ...
  implements ICommunityReviewRepository
{
  ...
  async upsertTrackerReview(input: UpsertCommunityTrackerReviewInput) {
    return this.execute(
      ...
      async () => {
        ...
        return this._mapper.toReviewEntity(
          review,
          input.userId,
  ...
  async toggleReviewHelpful(reviewId: string, userId: string) {
    return this.execute(
      ...
      async () => {
        if (
          !MongoCommunityObjectId.isValid(reviewId) ||
          !MongoCommunityObjectId.isValid(userId)
        ) {
          return null
        }
        ...
        const helpfulUserIds = this._mapper.toHelpfulUserIdStrings(
          review.helpfulUserIds,
        )
        const hasHelpful = helpfulUserIds.has(userId)

        const updatedReview =
          await CommunityTrackerReviewModel.findOneAndUpdate(
            {
              _id: reviewObjectId,
              deletedAt: null,
            },
  ...
  private async findReviews(
    ...
    const reviews = (await CommunityTrackerReviewModel.find({
      ...
      .lean<
        MongoCommunityTrackerReviewRecord[]
      >()) as MongoCommunityTrackerReviewRecord[]
  ...
  private async getRatingSummary(
    ...
    const rows =
      (await CommunityTrackerReviewModel.aggregate<MongoRatingAggregate>([
        {
          $match: {
            trackerId: MongoCommunityObjectId.toExistingObjectId(trackerId),
            deletedAt: null,
    ...
    for (const row of rows) {
      const rating = Math.min(Math.max(Number(row._id), 1), 5) as
        1 | 2 | 3 | 4 | 5
  ...
  private async toReviewEntities(
    ...
    return reviews
      .map((review) =>
        this._mapper.toReviewEntity(review, currentUserId, authors),
      )
      .filter((review): review is CommunityTrackerReviewEntity =>
        Boolean(review),
      )
  }
  ...
  private async findAuthors(
    ...
    const objectIds = userIds.map((userId) =>
      MongoCommunityObjectId.toObjectId(userId),
    )
