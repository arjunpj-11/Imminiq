...
export class GetRoadmapEvaluationResultUseCase implements IGetRoadmapEvaluationResultUseCase {
  ...
  async execute(jobId: string, userId: string): Promise<IGetEvaluationResultDTO> {
    ...
    if (!job.isEvaluationJob()) {
      throw OnboardingApplicationError.invalidJobType('This job is not a roadmap evaluation job');
    }

    if (!job.isCompleted()) {
      throw OnboardingApplicationError.jobPending('Evaluation is not completed yet');
    }

    const evaluation = this._onboardingJobOutputReader.getEvaluation(job.outputData);
