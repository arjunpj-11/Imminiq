...
export class MongoAdaptiveLearningRepository implements IAdaptiveLearningRepository {
  constructor(
    private readonly _mapper: MongoAdaptiveLearningMapper = new MongoAdaptiveLearningMapper()
  ) {}
  ...
  async getLearnerSnapshot(userId: string): Promise<AdaptiveLearnerSnapshot> {
    const [user, trackers, progressRows, reports] = await Promise.all([
      User.findById(userId).select('fullName xp level streakCount').lean<{
        fullName: string;
        xp: number;
        level: number;
        streakCount: number;
      ...
      Tracker.find({ ownerId: userId, deletedAt: null })
        ...
        >(),
      TrackerProgress.find({ userId }).select('trackerId completionPercentage lastStudiedAt').lean<
        Array<{
      ...
      >(),
      MockTestReportModel.find({ userId }).sort({ createdAt: -1 }).limit(12).lean<
        Array<{
    ...
    const testTitles = new Map(tests.map((test) => [test._id.toString(), test.title]));
    const progressByTracker = new Map(progressRows.map((row) => [row.trackerId.toString(), row]));
    const scoreTotal = reports.reduce((total, report) => total + report.scorePercentage, 0);
    ...
    return {
      ...
      trackers: trackers.map((tracker) => {
        ...
        return {
          ...
          ...(progress?.lastStudiedAt ? { lastStudiedAt: progress.lastStudiedAt } : {}),
        };
      }),
      ...
      averageScore: reports.length > 0 ? Math.round(scoreTotal / reports.length) : null,
    };
  }
  ...
  async listAdvisorMessages(userId: string, limit = 20): Promise<AdaptiveAdvisorMessage[]> {
    const documents = await AdaptiveAdvisorMessageModel.find({ userId })
      .sort({ createdAt: -1 })
      .limit(limit)
      .lean<MongoAdaptiveAdvisorMessageRecord[]>();

    return documents.reverse().map((document) => this._mapper.toAdvisorMessage(document));
  }
  ...
  async recordAssessmentResult(
    ...
    const { change, masteryScore: nextMasteryScore, level: nextLevel } = masteryResult;
...
export const mongoAdaptiveLearningRepository = new MongoAdaptiveLearningRepository();
