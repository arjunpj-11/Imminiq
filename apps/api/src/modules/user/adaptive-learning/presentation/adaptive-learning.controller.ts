...
export class AdaptiveLearningController {
  ...
  getDashboard = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const data = await this._useCases.getDashboard.execute(getAuthUser(req).userId);
  ...
  generateAssessment = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const data = await this._useCases.generateAssessment.execute(getAuthUser(req).userId);
