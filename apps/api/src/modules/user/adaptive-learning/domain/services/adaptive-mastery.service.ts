...
export const adaptiveMasteryLevelFor = (score: number): AdaptiveMasteryLevel => {
...
export const calculateAdaptiveMasteryResult = (input: {
  ...
  const magnitude =
    scoreDifference === 0 ? 0 : Math.max(1, Math.min(8, Math.round(Math.abs(scoreDifference) / 5)));
  const change = Math.sign(scoreDifference) * magnitude;
  const masteryScore = Math.max(0, Math.min(100, input.currentMasteryScore + change));
