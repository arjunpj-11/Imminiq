...
const trackerIntakeResponseSchema = z.object({
  ...
  profile: z
    .object({
      ...
      preferredLanguage: z.string().min(2).max(80),
...
export class TrackerIntakeAgent implements ITrackerIntakeAgent {
  async continueIntake(userId: string, messages: TrackerIntakeMessage[]) {
    ...
    const [user, trackers, recentReports] = await Promise.all([
      ...
      Tracker.find({ ownerId: userId, deletedAt: null })
        ...
        .sort({ updatedAt: -1 })
        .limit(env.TRACKER_INTAKE_CONTEXT_TRACKER_LIMIT)
        .lean(),
      MockTestReportModel.find({ userId })
        .select('scorePercentage weakTopics strongTopics')
        .sort({ createdAt: -1 })
        .limit(env.TRACKER_INTAKE_CONTEXT_TEST_REPORT_LIMIT)
        .lean(),
    ]);
    ...
    try {
      const response = await economyAIStructuredWithFallback(
        [
          {
            ...
            content: [
              ...
              'Gather: the precise subject, why it matters, desired outcome, existing experience or current level, weekly time, preferred language for roadmap content, preferred learning style, and important constraints or deadline.',
              ...
              'Never complete without a preferred content language. Ask for it directly when it has not been provided.',
              'Mark isComplete true once there is enough information for a personalized roadmap, normally after 3-7 user answers.',
              'When complete, include the full profile and use assistantMessage to briefly confirm what you understood.',
              `There have been ${userAnswerCount} user answers. If there are 7 or more, complete using the best available information instead of continuing indefinitely; use English only if the learner still did not specify a language.`,
              `Prior learner data: ${JSON.stringify(learnerContext)}.`,
              'Return only JSON matching this shape: {"assistantMessage": string, "isComplete": boolean, "profile"?: {"topic": string, "motivation": string, "desiredOutcome": string, "currentExperience": string, "weeklyTimeCommitment": string, "preferredLanguage": string, "learningPreferences": string[], "constraints": string[], "inferredLevel": "beginner" | "intermediate" | "advanced"}}.',
            ].join(' '),
          },
        ...
        'fast',
        'roadmap_generation',
        { operation: 'tracker-intake-agent', temperature: env.AI_TRACKER_INTAKE_TEMPERATURE }
      );
      ...
      const fallbackQuestions = [
        ...
        'Which language would you prefer for your roadmap and learning content?',
...
export const trackerIntakeAgent = new TrackerIntakeAgent();
