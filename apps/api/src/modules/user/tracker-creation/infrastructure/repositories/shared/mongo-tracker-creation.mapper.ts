...
import { TrackerCreationResponseEntity } from '../../../domain/entities/tracker-creation-response.entity';
import {
...
import type {
  ...
  MongoIdLike,
  MongoTrackerCreationResponseRecord,
  MongoRoadmapSubtopicRecord,
  MongoRoadmapTopicRecord,
  MongoTrackerRecord,
} from './mongo-tracker-creation.types';

export class MongoTrackerCreationMapper {
  ...
  toTrackerCreationResponseEntity(
    response: MongoTrackerCreationResponseRecord | null
  ): TrackerCreationResponseEntity | null {
    if (!response) {
      return null;
    }
    ...
    return new TrackerCreationResponseEntity({
      ...(id ? { id } : {}),
      ...(userId ? { userId } : {}),
  ...
  private toAIJobType(value: string): AIGenerationJobType {
    ...
    throw new TrackerCreationDomainError('INVALID_AI_JOB_TYPE', 'Stored AI job type is invalid');
  }
