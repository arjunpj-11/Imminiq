...
export class MongoTrackerProgressRepository extends MongoTrackerBaseRepository {
  ...
  async ensureUserProgressInitialized(data: EnsureUserProgressInitializedInput) {
    return this.execute(
    ...
    return this.execute(
      ...
      async () => {
        ...
        const progressMap = new Map(
          userProgress.map((progress) => [progress.subtopicId?.toString?.() ?? '', progress])
        );

        return subtopics.map((subtopic) =>
          this.mapper.toSubtopicWithProgress(subtopic, progressMap.get(subtopic._id.toString()))
        );
      }
    );
  }
    ...
    return this.execute(
      ...
      async () => {
        ...
        const wasNewlyCompleted =
          data.status === 'completed' && previousProgress?.status !== 'completed';
        ...
        return {
          subtopic: this.mapper.toSubtopicWithProgress(subtopic, userProgress),
  ...
  async unlockNextSubtopic(data: UnlockNextSubtopicInput) {
    return this.execute(
      ...
      async () => {
        ...
        const unlockedProgress = await UserSubtopicProgress.findOneAndUpdate(
          this.mapper.asMongoFilter({
            userId: userObjId,
            trackerId: trackerObjId,
        ...
        return {
          ...
          subtopicId: nextSubtopic._id.toString(),
        };
      }
    );
  }

  async checkAndCompleteParentSubtopic(data: CheckAndCompleteParentSubtopicInput) {
    return this.execute(
      ...
      async () => {
        ...
        const progressPercent = Math.round((completedCount / allChildren.length) * 100);
        ...
        const parentProgressFilter = this.mapper.asMongoFilter({
          userId: userObjId,
          trackerId: trackerObjId,
          subtopicId: parentObjId,
        });

        let completedParent = await UserSubtopicProgress.findOneAndUpdate(
          this.mapper.asMongoFilter({
            userId: userObjId,
            trackerId: trackerObjId,
            subtopicId: parentObjId,
        ...
        let wasNewlyCompleted = Boolean(completedParent);
        ...
        if (!completedParent) {
          completedParent = await UserSubtopicProgress.findOneAndUpdate(
            parentProgressFilter,
            this.mapper.asMongoUpdate({
              $setOnInsert: {
                userId: userObjId,
          ...
          wasNewlyCompleted = Boolean(completedParent);
        }
        ...
        return {
          ...
          isCompleted: completedParent?.status === 'completed',
  ...
  async checkAndCompleteTopicAndUnlockNext(data: CheckAndCompleteTopicAndUnlockNextInput) {
    return this.execute(
      ...
      async () => {
        ...
        const [allSubtopics, currentTopic] = await Promise.all([
          TrackerSubtopic.find(
            this.mapper.asMongoFilter({
              trackerId: trackerObjId,
              topicId: topicObjId,
        ...
        const topicTitle = typeof currentTopic?.title === 'string' ? currentTopic.title : null;
        ...
        const progressPercent = Math.round((completedCount / allSubtopics.length) * 100);

        if (completedCount < allSubtopics.length) {
        ...
        let wasNewlyCompleted = Boolean(completedTopic);

        if (!completedTopic) {
          completedTopic =
            await UserTopicProgress.findOne(topicProgressFilter).lean<MongoTopicProgressRecord>();
        }
        ...
        if (!completedTopic) {
          ...
          wasNewlyCompleted = Boolean(completedTopic);
        }
        ...
        if (!currentTopic) {
          return {
            ...
            isCompleted: completedTopic?.status === 'completed',
        ...
        return {
          ...
          nextTopicId: nextTopic._id.toString(),
        };
      }
    );
  }
  ...
  async recomputeTrackerProgress(data: RecomputeTrackerProgressInput) {
    return this.execute(
      ...
      async () => {
        ...
        const [totalSubtopics, completedSubtopics, totalTopics, completedTopics, previousTracker] =
          await Promise.all([
            TrackerSubtopic.countDocuments(
              this.mapper.asMongoFilter({
                trackerId: trackerObjId,
        ...
        const wasNewlyCompleted = isCompleted && previousTracker?.status !== 'completed';
...
export const mongoTrackerProgressRepository = new MongoTrackerProgressRepository();
