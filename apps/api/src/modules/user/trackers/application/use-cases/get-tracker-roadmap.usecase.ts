...
const buildRoadmapTree = ({
  ...
  for (const subtopic of subtopics) {
    const parentKey =
      subtopic.parentSubtopicId?.toString() || `topic:${subtopic.topicId.toString()}`;
  ...
  const attachChildren = (items: RoadmapSubtopicNode[]): RoadmapSubtopicNode[] => {
    return items
  ...
  return topics
    .sort((first, second) => first.order - second.order)
    .map((topic) => {
      const topicChildren = subtopicMap.get(`topic:${topic._id.toString()}`) || [];
