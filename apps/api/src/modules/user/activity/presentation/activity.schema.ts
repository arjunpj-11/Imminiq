...
export const activityPageQuerySchema = z.object({
  year: z.coerce.number().int().min(ACTIVITY_MIN_YEAR).max(currentYear).default(currentYear),

  ...feedFields,
});

export const activityFeedQuerySchema = z.object(feedFields);

export type ActivityPageQueryInput = z.infer<typeof activityPageQuerySchema>;
