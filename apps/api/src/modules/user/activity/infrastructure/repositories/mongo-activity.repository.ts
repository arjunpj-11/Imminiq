import type { RecordUserActivityInput } from '../../domain/repositories/activity-command.repository.interface';
...
export class MongoActivityRepository implements IActivityRepository {
  constructor(
    private readonly _queryRepository = new MongoActivityQueryRepository(),
    private readonly _commandRepository = new MongoActivityCommandRepository()
  ) {}
  ...
  findActivityAnalytics(input: FindActivityAnalyticsInput) {
    return this._queryRepository.findActivityAnalytics(input);
  }

  findDailyGoalState(input: FindDailyGoalStateInput) {
    return this._queryRepository.findDailyGoalState(input);
  }

  recordActivityAndApplyReward(input: RecordUserActivityInput) {
    return this._commandRepository.recordActivityAndApplyReward(input);
  }
}
