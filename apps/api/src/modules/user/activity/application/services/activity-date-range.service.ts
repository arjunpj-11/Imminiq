...
export class ActivityDateRange {
  createContext(now: Date, requestedYear?: number, utcOffsetMinutes = 0): ActivityDateContext {
    ...
    if (!Number.isInteger(year) || year < ACTIVITY_MIN_YEAR || year > currentLocalYear) {
    ...
    const localDayOfWeek = new Date(localTodayStart).getUTCDay();

    const daysSinceMonday = localDayOfWeek === 0 ? 6 : localDayOfWeek - 1;

    const localCurrentWeekStart = localTodayStart - daysSinceMonday * DAY_IN_MS;

    const currentWeekRange = {
      start: this.unshift(new Date(localCurrentWeekStart), utcOffsetMinutes),
      end: this.unshift(new Date(localCurrentWeekStart + 7 * DAY_IN_MS), utcOffsetMinutes),
    };

    const previousWeekRange = {
      start: new Date(currentWeekRange.start.getTime() - 7 * DAY_IN_MS),
      end: new Date(currentWeekRange.start),
    };

    const todayRange = {
      start: this.unshift(new Date(localTodayStart), utcOffsetMinutes),
      end: this.unshift(new Date(localTodayStart + DAY_IN_MS), utcOffsetMinutes),
    };

    const yearRange = {
      start: this.unshift(new Date(Date.UTC(year, 0, 1)), utcOffsetMinutes),
      end: this.unshift(new Date(Date.UTC(year + 1, 0, 1)), utcOffsetMinutes),
    };
    ...
    return {
      ...
      yesterdayKey: this.toLocalDateKey(new Date(now.getTime() - DAY_IN_MS), utcOffsetMinutes),
      ...
      currentWeekDateKeys: Array.from({ length: 7 }, (_, index) =>
        this.toLocalDateKey(
          new Date(currentWeekRange.start.getTime() + index * DAY_IN_MS),
          utcOffsetMinutes
        )
      ),
    };
  ...
  toLocalDateKey(date: Date, utcOffsetMinutes: number): string {
    return this.shift(date, utcOffsetMinutes).toISOString().slice(0, 10);
  }
  ...
  groupLabel(
    dateKey: string,
    context: Pick<ActivityDateContext, 'todayKey' | 'yesterdayKey'>
  ): string {
