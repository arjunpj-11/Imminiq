...
import { ActivityMapper } from './application/activity.mapper';
import { ActivityEventPolicy } from './application/activity-event.policy';
...
export const createActivityComposition = (): ActivityComposition => {
  const activityRepository = mongoActivityRepository;

  const mapper = new ActivityMapper();
  const eventPolicy = new ActivityEventPolicy();
  const analyticsCalculator = new ActivityAnalytics();
  const cursorCodec = new ActivityCursorCodec();
  const dateRange = new ActivityDateRange();
  ...
  return {
    useCases: {
      ...
      recordActivity: new RecordUserActivityUseCase(
        activityRepository,
        eventPolicy,
        mapper,
        dateRange,
