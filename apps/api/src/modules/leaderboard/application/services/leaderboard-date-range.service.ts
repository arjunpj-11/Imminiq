  ...
  implements LeaderboardDateRangeContract
{
  getPeriods(now: Date): LeaderboardPeriods {
    ...
    const elapsedInCurrentWeek = Math.max(
      0,
    ...
    const previousEnd = new Date(
      Math.min(
        ...
        previousStart.getTime() + elapsedInCurrentWeek,
      ),
    )
