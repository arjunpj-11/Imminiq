...
export type FriendsMongoErrorMapper = (
  error: unknown,
) => FriendsDomainError | null

export class MongoFriendsErrorMapper {
  static mapDuplicateRelationshipError(
    error: unknown,
  ): FriendsDomainError | null {
    /*
     ...
     */
    if (
      !MongoFriendsErrorMapper.isDuplicateKeyError(
        error,
      )
    ) {
      return null
    ...
    if (
      'userId' in duplicateKeys ||
      'friendId' in duplicateKeys
    ) {
      return new FriendsDomainError(
        'FRIENDSHIP_CONFLICT',
    ...
    return new FriendsDomainError(
      'FRIENDS_DUPLICATE_RECORD',
      'Friends record already exists',
    )
  }
