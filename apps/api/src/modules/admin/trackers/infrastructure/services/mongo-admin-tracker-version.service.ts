...
export class AdminTrackerVersionService implements IAdminTrackerVersionService {
  async list(trackerId: string) {
    const rows = await TrackerVersion.find({ trackerId })
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        changedBy: actor?.fullName ?? actor?.username ?? 'Unknown',
        reason: row.reason,
    ...
    try {
      ...
      await session.withTransaction(async () => {
        const [target, current] = await Promise.all([
          TrackerVersion.findOne({ trackerId, version }).session(session).lean(),
          Tracker.findById(trackerId).session(session),
        ]);
        if (!target || !current)
          throw new ServiceError(
            'missing-resource',
            'TRACKER_VERSION_NOT_FOUND',
            'Tracker version not found'
          );
        await TrackerVersion.updateOne(
          { trackerId, version: current.version ?? 1 },
          {
            $setOnInsert: {
              snapshot: current.toObject(),
              changedBy: actor.userId,
              reason: 'Snapshot before admin restore',
            },
          },
          { upsert: true, session }
        );
        ...
        await recordAdminAction(
          actor,
          'tracker.version_restored',
          'trackers',
          { trackerId, restoredVersion: version, newVersion: current.version, reason },
          session
        );
        output = {
          trackerId,
