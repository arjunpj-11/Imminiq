...
export const adminTrackerBulkLifecycleSchema = adminTrackerLifecycleSchema.extend({ ids: z.array(z.string().regex(/^[a-f\d]{24}$/i)).min(1).max(100), preview: z.boolean().default(true) });
export const adminTrackerVersionRestoreSchema = z.object({ reason: z.string().trim().min(10).max(1000) });
