...
export interface IAdminMockTestsUseCase {
  list(query: AdminListQuery): Promise<AdminPage<AdminMockTest>>;
  getDetail(id: string): Promise<AdminMockTestDetail>;
}
export class AdminMockTestsUseCase implements IAdminMockTestsUseCase {
  ...
  async getDetail(id: string) {
    ...
    if (!test) throw new ApiError(404, 'Mock test not found', 'MOCK_TEST_NOT_FOUND');
    return test;
  }
}
