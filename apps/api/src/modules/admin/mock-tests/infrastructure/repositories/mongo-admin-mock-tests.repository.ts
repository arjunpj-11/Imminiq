...
export class MongoAdminMockTestsRepository implements IAdminMockTestsRepository {
  async list(query: AdminListQuery) {
    ...
    if (query.status && query.status !== 'all') filter.visibility = query.status;
    if (query.search) filter.title = new RegExp(escapeAdminSearch(query.search), 'i');
    const [rows, total, publicCount, privateCount, attempts] = await Promise.all([
      ...
      MockTestModel.countDocuments({ visibility: 'public' }),
      MockTestModel.countDocuments({ visibility: 'private' }),
  ...
  async getDetail(id: string) {
    const [test, questions] = await Promise.all([
      MockTestModel.findById(id).populate('ownerId', 'fullName username').lean(),
      MockTestQuestionModel.find({ testId: id }).sort({ order: 1 }).lean(),
    ]);
    if (!test) return null;
    const owner = test.ownerId as unknown as { fullName?: string; username?: string };
    return {
      id: String(test._id),
      title: test.title,
