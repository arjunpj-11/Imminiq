...
export class AdminUserNotesService implements IAdminUserNotesService {
  async list(userId: string) {
    ...
    return {
      tags: user?.adminTags ?? [],
      notes: notes.map((row) => {
        const author = row.authorId as unknown as { fullName?: string; username?: string };
        return {
          id: String(row._id),
          note: row.note,
          tags: row.tags,
          author: author?.fullName ?? author?.username ?? 'Admin',
          createdAt: row.createdAt,
        };
      }),
    };
  }
  async add(userId: string, input: AdminUserNoteInput, actor: AdminActor) {
    if (!(await User.exists({ _id: userId, deletedAt: null })))
      throw new ServiceError('missing-resource', 'USER_NOT_FOUND', 'User not found');
    ...
    await recordAdminAction(actor, 'user_note.created', 'users', {
      userId,
      noteId: String(row._id),
      tags: input.tags,
    });
  ...
  async remove(userId: string, noteId: string, actor: AdminActor) {
    const row = await AdminUserNote.findOneAndUpdate(
      { _id: noteId, userId, deletedAt: null },
      { $set: { deletedAt: new Date() } },
      { new: true }
    );
    if (!row)
      throw new ServiceError('missing-resource', 'USER_NOTE_NOT_FOUND', 'User note not found');
  ...
  async updateTags(userId: string, tags: string[], actor: AdminActor) {
    const user = (await User.findByIdAndUpdate(userId, { $set: { adminTags: tags } }, { new: true })
      .select('adminTags')
      .lean()) as { adminTags?: string[] } | null;
