...
export class AdminUsersController {
  ...
  list = async (req: Request, res: Response, next: NextFunction) => {
    try {
      res.json(
        new ApiResponse(
          'Users fetched',
          await this._useCases.list.execute(adminUsersQuerySchema.parse(req.query))
        )
      );
    } catch (error) {
      next(error);
  ...
  getDetail = async (req: Request<UserIdParams>, res: Response, next: NextFunction) => {
    try {
      res.json(
        new ApiResponse(
          'User detail fetched',
          await this._useCases.getDetail.execute(req.params.userId)
        )
      );
    } catch (error) {
      next(error);
  ...
  setStatus = async (req: Request<UserIdParams>, res: Response, next: NextFunction) => {
    try {
      ...
      const data = await this._useCases.setStatus.execute(
        req.params.userId,
        payload.status,
      ...
      res.json(
        new ApiResponse(`User ${payload.status === 'blocked' ? 'blocked' : 'unblocked'}`, data)
      );
    } catch (error) {
      next(error);
    }
