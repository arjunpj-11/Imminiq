...
export interface ISetAdminUserStatusUseCase { execute(userId: string, status: AdminManagedUserStatus, actor: AdminActor, meta: AdminActionMeta): Promise<IAdminStatusResultDTO> }
export class SetAdminUserStatusUseCase implements ISetAdminUserStatusUseCase {
  ...
  async execute(userId: string, status: AdminManagedUserStatus, actor: AdminActor, meta: AdminActionMeta) {
    if (!/^[a-f\d]{24}$/i.test(userId)) throw new AdminUsersDomainError('INVALID_USER_ID', 'Invalid user ID')
    ...
    if (target.role === 'superadmin') throw AdminUsersApplicationError.protectedAdmin('A super admin cannot be blocked')
