...
export class MongoAdminDashboardRepository implements IAdminDashboardRepository {
  async getOverview(): Promise<AdminDashboardEntity> {
    ...
    const [totalUsers, activeToday, blockedUsers, totalTrackers, recentActivity, weeklyActivity] = await Promise.all([
    ...
    const activityUsers = await User.find({ _id: { $in: recentActivity.map((item) => item.userId) } }).select('fullName username').lean()
    const userById = new Map(activityUsers.map((user) => [String(user._id), { fullName: user.fullName, username: user.username }]))
    ...
    return {
      ...
      recentActivity: recentActivity.map((item) => ({ id: String(item._id), action: item.action, module: item.module, severity: item.severity, createdAt: item.createdAt, user: userById.get(String(item.userId)) ?? null })),
    }
  }
}
