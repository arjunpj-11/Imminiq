...
const csv = (rows: unknown[][]) =>
  rows.map((row) => row.map(safeAdminCsvValue).join(',')).join('\r\n');
const search = (value: string, fields: string[]) =>
  value
    ? {
        $or: fields.map((field) => ({
          [field]: { $regex: escapeAdminExportSearch(value), $options: 'i' },
        })),
      }
    : {};
...
export class AdminExportService implements IAdminExportService {
  async users(query: { search: string; status: string }) {
    const filter: Record<string, unknown> = {
      deletedAt: null,
      ...search(query.search, ['fullName', 'username', 'email', 'phone']),
    };
    ...
    const rows = await User.find(filter)
      ...
      .limit(50_000)
      .select(
        'fullName username email phone role status emailVerified phoneVerified lastActiveAt createdAt'
      )
      .lean();
    return csv([
      ['ID', 'Name', 'Username', 'Email', 'Phone', 'Role', 'Status', 'Created', 'Last active'],
      ...rows.map((row) => [
        row._id,
        row.fullName,
        row.username,
  ...
  async trackers(query: { search: string; status: string }) {
    const filter: Record<string, unknown> = {
      ...
      ...search(query.search, ['title', 'description', 'tags']),
    };
    ...
    const rows = await Tracker.find(filter)
    ...
    return csv([
      ...
      ...rows.map((row) => {
        const owner = row.ownerId as unknown as { fullName?: string; email?: string };
        return [
          row._id,
          row.title,
          owner?.fullName,
  ...
  async mockTests(query: { search: string; status: string }) {
    ...
    const rows = await MockTestModel.find(filter)
      .populate('ownerId', 'fullName username email')
      .sort({ createdAt: -1 })
      .limit(50_000)
      .lean();
