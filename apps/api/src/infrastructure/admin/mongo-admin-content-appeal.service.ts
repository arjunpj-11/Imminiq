...
export class AdminContentAppealService implements IAdminContentAppealService {
  async list(targetType: AdminContentAppealTargetType, query: AdminContentAppealListQuery) {
    ...
    const [rows, total, stats] = await Promise.all([
      ContentModerationAppeal.find(filter)
      ...
      ContentModerationAppeal.aggregate<{ _id: string; count: number }>([
        { $match: { targetType, deletedAt: null } },
    ...
    const targets =
      targetType === 'tracker'
        ? await Tracker.find({ _id: { $in: targetIds } })
            .select('title moderationStatus')
            .lean()
        : await MockTestModel.find({ _id: { $in: targetIds } })
            .select('title moderationStatus')
            .lean();
    ...
    return {
      items: rows.map((row) => {
        const owner = row.ownerId as unknown as {
          _id: unknown;
          fullName?: string;
          username?: string;
          email?: string;
        ...
        return {
          ...
          targetType,
          title: target?.title ?? 'Removed content',
          moderationStatus: target?.moderationStatus ?? 'deleted',
          ownerId: String(owner?._id ?? ''),
          ownerName: owner?.fullName ?? owner?.username ?? 'Unknown owner',
  ...
  async update(
    ...
    try {
      ...
      await session.withTransaction(async () => {
        const row = await ContentModerationAppeal.findOne({
          _id: id,
          targetType,
          deletedAt: null,
        }).session(session);
        ...
        await recordAdminAction(
          actor,
          'content_appeal.updated',
          targetType === 'tracker' ? 'trackers' : 'mock-tests',
          { appealId: id, targetId: String(row.targetId), status: input.status },
          session
        );
        result = {
          id,
