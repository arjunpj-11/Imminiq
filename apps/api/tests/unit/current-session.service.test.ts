...
describe('CurrentSessionResolver', () => {
  it('uses the authenticated access-token session id without reading a scoped refresh cookie', async () => {
    ...
    const resolver = new CurrentSessionResolver(
      repository as never,
      hasher as never,
    )

    await expect(
      resolver.getCurrentSessionId(undefined, 'active-session-id'),
    ).resolves.toBe('active-session-id')
  ...
  it('keeps refresh-token lookup as a fallback for legacy sessions', async () => {
    const repository = {
      findCurrentSessionByRefreshTokenHash: vi
        .fn()
        .mockResolvedValue({ id: 'legacy-session-id' }),
    }
    ...
    await expect(
      resolver.getCurrentSessionId('raw-refresh-token'),
    ).resolves.toBe('legacy-session-id')
